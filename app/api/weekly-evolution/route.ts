...
function mondayFromKey(key: string): Date {
...
function weeksBetweenKeys(keyA: string, keyB: string): number {
  const a = mondayFromKey(keyA).getTime()
  const b = mondayFromKey(keyB).getTime()
  return Math.round((b - a) / (7 * DAY_MS))
}

function labelFromMonday(monday: Date): string {
...
function toComputeTrade(row: TradeRow): Trade | null {
...
function buildWeekPoint(weekKey: string, trades: Trade[]): WeekPoint {
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Rattachement par semaine ISO (date de clôture, fallback ouverture)
    ...
    return NextResponse.json({ weeksActiveCount, trajectory, current, gapWeeks })
  } catch (error: any) {
