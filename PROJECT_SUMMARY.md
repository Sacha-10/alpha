# RÉSUMÉ COMPLET — AlphaTradeX (à jour au 20 juillet 2026)

---

## 0. CE QU'EST ALPHATRADEX

**AlphaTradeX** est un SaaS d'**analyse IA de l'historique de trading**. Le membre importe l'historique de son compte (fichier CSV exporté depuis MT4, MT5, Binance, Bybit, TradingView, FTMO ou FundedNext — aucun accès au compte), et l'IA (`gpt-5.6-sol`) en produit en ~60 secondes un rapport structuré : scores (psychologie, gestion du risque, Prop Firm Readiness), statistiques calculées côté serveur, biais comportementaux détectés algorithmiquement, performance par session (Londres / New York / Tokyo), patterns temporels et plan d'action personnalisé.

**Cibles** : traders actifs cherchant à corriger leurs biais, et candidats **prop firm** (FTMO / FundedNext) évaluant leur admissibilité. **Objectif produit** : transformer un historique brut en « miroir sans filtre » exploitable, pour ne plus répéter les mêmes erreurs. Une **démo gratuite** (1 analyse par IP, données factices) est ouverte sans inscription ; l'accès complet est vendu par abonnement (plans Pro / Premium / Élite, Stripe). Stack : Next.js 16 (App Router), Supabase (auth Google + Postgres), OpenAI, Stripe, Resend, déploiement Vercel.

---

## 1. STACK ET DÉPENDANCES

### Dependencies (production)
| Package | Version |
|---|---|
| `next` | ^16.2.3 |
| `react` | ^18.3.1 |
| `react-dom` | ^18.3.1 |
| `@supabase/supabase-js` | ^2.47.10 |
| `@supabase/ssr` | ^0.10.2 |
| `stripe` | ^17.5.0 |
| `openai` | ^4.77.3 |
| `framer-motion` | ^11.15.0 |
| `lucide-react` | ^0.468.0 |
| `nextjs-toploader` | ^3.9.17 |
| `@radix-ui/react-popover` | ^1.1.15 |
| `@sparticuz/chromium` | ^148.0.0 |
| `puppeteer-core` | ^24.43.0 |
| `resend` | ^6.12.3 |

### DevDependencies
| Package | Version |
|---|---|
| `typescript` | ^5.7.2 |
| `tailwindcss` | ^3.4.17 |
| `autoprefixer` | ^10.4.20 |
| `postcss` | ^8.4.49 |
| `cross-env` | ^10.1.0 |
| `eslint` | ^8.57.1 |
| `eslint-config-next` | ^14.2.21 |
| `@types/node` | ^20.17.12 |
| `@types/react` | ^18.3.18 |
| `@types/react-dom` | ^18.3.5 |

### Scripts npm
- `dev` → `cross-env TURBOPACK= next dev` (Turbopack désactivé explicitement)
- `dev:turbo` → `next dev --turbo`
- `build` / `start` / `lint`

---

## 2. STRUCTURE DES FICHIERS

```
alpha/
├── app/
│   ├── icon.svg                        ← favicon (logo AlphaTradeX, identique à public/logo.svg)
│   ├── apple-icon.svg                  ← icône Apple Touch (identique à public/logo.svg)
│   ├── globals.css
│   ├── layout.tsx                      ← Root layout global
│   ├── page.tsx                        ← Page d'accueil /
│   ├── not-found.tsx                   ← Page 404 personnalisée
│   ├── opengraph-image.tsx             ← image OG (convention Next.js)
│   ├── twitter-image.tsx               ← image Twitter Card (convention Next.js)
│   ├── robots.ts                       ← robots.txt (convention Next.js)
│   ├── sitemap.ts                      ← sitemap.xml (convention Next.js)
│   ├── api/
│   │   ├── analyze/route.ts            ← POST (auth requise, rate-limit 15s)
│   │   ├── analyze-demo/route.ts       ← POST (rate-limit par IP)
│   │   ├── analyses/route.ts           ← GET historique analyses (rétention par plan)
│   │   ├── auth/callback/route.ts      ← OAuth callback Supabase (@supabase/ssr)
│   │   ├── create-checkout/route.ts    ← Stripe checkout (GET, param token)
│   │   ├── cron/weekly-summary-email/route.ts ← Cron Vercel email Résumé semaine (CRON_SECRET)
│   │   ├── customer-portal/route.ts    ← Portail client Stripe (GET)
│   │   ├── email/unsubscribe/route.ts  ← Désinscription un-clic email (token signé)
│   │   ├── generate-pdf/route.tsx      ← POST export PDF (Puppeteer + @sparticuz/chromium)
│   │   ├── import-trades/route.ts      ← POST import trades bruts (table trades)
│   │   ├── trades/route.ts             ← GET trades (journal de trades)
│   │   ├── webhook/route.ts            ← Webhook Stripe (3 événements + email Resend)
│   │   ├── weekly-evolution/route.ts   ← GET données Évolution semaine (Premium+)
│   │   └── weekly-summary/route.ts     ← GET Résumé semaine (Premium+)
│   ├── about/
│   │   ├── layout.tsx                  ← metadata dédiée (title/description)
│   │   └── page.tsx                    ← /about (Page À propos)
│   ├── analysis/
│   │   ├── layout.tsx                  ← metadata dédiée (title/description)
│   │   └── page.tsx                    ← /analysis (démo libre, 1/IP)
│   ├── dashboard/
│   │   ├── page.tsx                    ← Wrapper Suspense
│   │   └── DashboardClient.tsx         ← /dashboard (protégé par proxy.ts) — shell sidebar+topbar
│   ├── help/
│   │   ├── layout.tsx                  ← metadata dédiée (title/description)
│   │   └── page.tsx                    ← /help (Page Aide)
│   ├── pricing/
│   │   ├── layout.tsx                  ← metadata dédiée (title/description)
│   │   └── page.tsx                    ← /pricing
│   └── legal/
│       ├── layout.tsx                  ← Navbar + Footer partagés
│       ├── cgu/page.tsx
│       ├── cgv/page.tsx
│       ├── confidentialite/page.tsx
│       ├── cookies/page.tsx
│       ├── mentions-legales/page.tsx
│       └── risque/page.tsx
├── components/
│   ├── Navbar.tsx
│   ├── Footer.tsx
│   ├── CookieBanner.tsx
│   ├── Providers.tsx
│   ├── ScrollReset.tsx
│   ├── LogoSvg.tsx                     ← logo SVG inline (navbar, dashboard)
│   ├── RevealSection.tsx               ← reveal au scroll (pages marketing)
│   ├── TradeReport.tsx                 ← wrapper "use client" (animations, quota, PDF)
│   ├── TradeReportBody.tsx             ← contenu pur du rapport (partagé UI)
│   ├── TradeJournal.tsx                ← journal de trades (calendrier, import/export)
│   ├── AnalysisHistory.tsx             ← historique des analyses (accordéon)
│   ├── WeeklyEvolution.tsx             ← vue Évolution semaine (graphiques hebdo)
│   └── WeeklySummary.tsx               ← vue Résumé semaine
├── lib/
│   ├── supabase.ts
│   ├── openai.ts                       ← analyzeTradesDemo + analyzeTradesMember + computeStats
│   ├── plans.ts
│   ├── demoTrades.ts
│   ├── tradingAnalysisTypes.ts
│   ├── parseDbDate.ts                  ← lecture UTC des timestamps DB (ajoute 'Z' si aucun offset)
│   ├── resend.ts                       ← client Resend (RESEND_API_KEY)
│   ├── emailToken.ts                   ← token HMAC désinscription (UNSUBSCRIBE_SECRET)
│   ├── weeklyData.ts                   ← rows → WeekPoint (partagé Évolution/Résumé)
│   ├── weeklyFormat.ts                 ← formatage partagé des données hebdo
│   ├── weeklySummary.ts                ← dérivation du Résumé semaine (route + cron email)
│   ├── emails/
│   │   ├── confirmationPaiement.ts     ← template HTML email confirmation paiement
│   │   └── weeklySummaryEmail.ts       ← template HTML email Résumé semaine
│   └── parseCSV/
│       ├── index.ts
│       ├── utils.ts                    ← cleanSymbol, normalizeCSV, detectDelimiter, makeCSVParser, parseTradeDate (UTC)
│       ├── session.ts                  ← getSession (fenêtres UTC partagées des parseurs)
│       ├── sourceId.ts                 ← clé source_id de dédup à l'import
│       ├── parseMT4.ts
│       ├── parseTradingView.ts
│       ├── parseBinance.ts
│       ├── parseBybit.ts
│       └── parseFTMO.ts
├── styles/
│   └── theme.ts                        ← Source of truth couleurs/tokens
├── proxy.ts                            ← garde de statut centralisée (ex-middleware.ts, Next.js 16)
├── vercel.json                         ← cron Vercel : /api/cron/weekly-summary-email, schedule "0 0 * * 1" (lundi 00:00 UTC)
├── tailwind.config.ts
├── next.config.mjs
├── postcss.config.mjs
├── tsconfig.json                       ← strict: true, paths @/* → ./*, target ES2017
├── .eslintrc.json
├── .cursorignore                       ← règles « secrets » (ne jamais indexer .env*, *.pem, *.key)
├── CLAUDE.md / AGENTS.md
├── README.md                           ← boilerplate create-next-app intact (dette, cf. §8)
├── emails-preview/
│   └── confirmation.html               ← artefact de preview de l'email de confirmation (non branché, cf. §8)
└── public/
    ├── manifest.json                   ← PWA manifest
    ├── logo.svg
    └── logo.png                        ← utilisé dans l'email de confirmation paiement (lib/emails/confirmationPaiement.ts)
```

---

## 3. DESIGN SYSTEM COMPLET

### Source de vérité : `styles/theme.ts`

```ts
background:      "#0A0A0F"   // fond principal
backgroundCard:  "#12121A"   // fond des cards
backgroundHover: "#1A1A28"   // fond au hover
accentBlue:      "#2D6FFF"   // couleur principale
accentCyan:      "#00E5FF"   // accent secondaire
accentRed:       "#FF3D57"   // erreurs / danger
accentOrange:    "#FFB800"   // avertissements
accentGreen:     "#00E5B0"   // succès / positif
textPrimary:     "#F0F4FF"   // texte principal
textSecondary:   "#8892AA"   // texte secondaire
border:          "#1E2035"   // bordures
borderHover:     "#2D6FFF"   // bordures au hover
borderRadius:    "12px"
transition:      "200ms ease"
shadow:          "0 0 40px rgba(45, 111, 255, 0.07)"
```

### Variables CSS — `:root` dans `globals.css`

```css
--background:     #0A0A0F
--card:           #12121A
--hover:          #1A1A28
--blue:           #2D6FFF
--cyan:           #00E5FF
--red:            #FF3D57
--orange:         #FFB800
--green:          #00E5B0
--primary:        #F0F4FF
--secondary:      #8892AA
--border:         #1E2035
--radius:         12px
--transition:     200ms ease
--shadow:         0 0 40px rgba(45, 111, 255, 0.07)
--background-rgb: 10, 10, 15
--blue-rgb:       45, 111, 255
--cyan-rgb:       0, 229, 255
--red-rgb:        255, 61, 87
--orange-rgb:     255, 184, 0
--green-rgb:      0, 229, 176
--primary-rgb:    240, 244, 255
--secondary-rgb:  136, 146, 170
--border-rgb:     30, 32, 53
```

### Tailwind — Couleurs custom (`tailwind.config.ts`)

| Token Tailwind | Valeur hex |
|---|---|
| `background` | #0A0A0F |
| `card` | #12121A |
| `hover` | #1A1A28 |
| `blue` | #2D6FFF |
| `cyan` | #00E5FF |
| `red` | #FF3D57 |
| `orange` | #FFB800 |
| `green` | #00E5B0 |
| `primary` | #F0F4FF |
| `secondary` | #8892AA |
| `border` | #1E2035 |
| `borderHover` | #2D6FFF |

### Tailwind — Fonts
```ts
sans: ["var(--font-inter)", "sans-serif"]
mono: ["var(--font-jetbrains-mono)", "monospace"]
```

### Tailwind — Border radius
```ts
DEFAULT: "12px"
```

### Tailwind — Box shadows
```ts
card: "0 0 40px rgba(45, 111, 255, 0.07)"
blue: "0 0 20px rgba(45, 111, 255, 0.3)"
```

### Récapitulatif complet de toutes les valeurs rgba bleu du projet

| Valeur | Fichier | Rôle |
|---|---|---|
| `rgba(45, 111, 255, 0.07)` | `globals.css`, `theme.ts` | `--shadow` / cards |
| `rgba(45, 111, 255, 0.15)` | `globals.css` | `.glow-blue`, `.hover:glow-blue` |
| `rgba(45, 111, 255, 0.20)` | `app/layout.tsx`, `app/opengraph-image.tsx` | Glow radial décoratif global / fond de l'image OG |
| `rgba(45, 111, 255, 0.3)` | `globals.css`, `tailwind.config.ts` | `.btn-primary:hover`, `shadow-blue` |
| `rgba(var(--blue-rgb), 0.35)` | `Providers.tsx` | NextTopLoader shadow |

### Polices

- **Inter** : weights 400, 500, 600, 700 — variable CSS `--font-inter`
- **JetBrains Mono** : weights 400, 500 — variable CSS `--font-jetbrains-mono`
- Aucun `@import` Google Fonts dans `globals.css` — l'URL `https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap` n'est utilisée que par le template HTML du PDF (`app/api/generate-pdf/route.tsx`) et par `app/opengraph-image.tsx` (fetch au build). Fallback runtime réel : `font-family: "Inter", var(--font-inter), sans-serif` (`globals.css`)

### Animations

**`landing-marquee`** (`globals.css`) :
```css
@keyframes landing-marquee {
  from { transform: translate3d(0, 0, 0); }
  to   { transform: translate3d(-50%, 0, 0); }
}
.animate-landing-marquee {
  display: flex;
  width: max-content;
  animation: landing-marquee 50s linear infinite;
  will-change: transform;
  backface-visibility: hidden;
}
```

**Framer Motion** :
- `TradeReport.tsx` : `initial={{ opacity:0, y:20 }}` → `animate={{ opacity:1, y:0 }}` delays 0 / 0.1 / 0.2 / 0.3 / 0.35 / 0.4 / 0.45 / 0.5s
- `analysis/page.tsx` : même pattern + barre `width:['0%','90%']` duration 8s

**RevealSection** (IntersectionObserver threshold 0.15) :
```
invisible : translateY(28px) opacity-0   ← style inline UNIQUE pour tous les consommateurs (/, about, help, …)
visible   : translateY(0) opacity-100
transition: 700ms ease-out
```
One-shot : `observer.unobserve()` après premier déclenchement. Le fichier exporte aussi le hook `useReveal()`.

### Transition globale — conditionnée par `html.hydrated`

La transition CSS globale (color / background-color / border-color / box-shadow / opacity, 200ms ease) n'est PAS inconditionnelle : elle ne s'applique qu'une fois la classe `hydrated` posée sur `<html>` après le 1er paint (anti flash blanc iOS Safari). `prefers-reduced-motion: reduce` force `transition/animation-duration: 0.01ms !important` + `scroll-behavior: auto`.

> Bloc CSS complet, scripts inline et raison détaillée : **§6 « Mécanisme `.hydrated` »**.

### Classes CSS utilitaires globales
```css
.card          { background: #12121A; border: 1px solid #1E2035;
                 border-radius: 12px; box-shadow: var(--shadow) }
.card:hover    { border-color: #2D6FFF }

.btn-primary   { background: #2D6FFF; color: white; border-radius: 12px;
                 padding: 12px 24px; font-weight: 600; cursor: pointer }
.btn-primary:hover { opacity: 0.9; box-shadow: 0 0 20px rgba(45,111,255,0.3) }

.btn-outline   { background: transparent; color: #F0F4FF;
                 border: 1px solid #1E2035; border-radius: 12px;
                 padding: 12px 24px; font-weight: 600; cursor: pointer }
.btn-outline:hover { border-color: #2D6FFF; color: #2D6FFF }

.glow-blue     { box-shadow: 0 0 40px rgba(45,111,255,0.15) }
.hover\:glow-blue:hover { box-shadow: 0 0 40px rgba(45,111,255,0.15) }
```

### Scrollbar custom
```css
::-webkit-scrollbar        { width: 6px; height: 6px }
::-webkit-scrollbar-track  { background: #0A0A0F }
::-webkit-scrollbar-thumb  { background: #2D6FFF; border-radius: 3px }
scrollbar-color: var(--blue) var(--background)   /* Firefox */
scrollbar-width: thin
scrollbar-gutter: stable   /* uniquement sous @media (hover: hover) and (pointer: fine) — desktop */
::selection { background: #2D6FFF; color: #F0F4FF }
```

---

## 4. COMPOSANTS

### `Navbar.tsx` — `"use client"`

**Logo — composant partagé `components/LogoSvg.tsx` (prop unique `size`, défaut 32), importé par Navbar ET Footer (pas de SVG inline dupliqué) :**
```svg
<svg width="32" height="32" viewBox="0 0 600 600"
     style="border-radius:8px; flex-shrink:0" aria-hidden>
  <rect width="600" height="600" rx="125" ry="125" fill="#0A0A0F"/>
  <svg x="75" y="75" width="450" height="450" viewBox="0 0 24 24"
       fill="#0A0A0F" stroke="#2D6FFF" stroke-width="2"
       stroke-linecap="round" stroke-linejoin="round">
    <polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/>
    <polyline points="16 7 22 7 22 13"/>
  </svg>
</svg>
```

**`<header>` :**
- `position: fixed; inset-x: 0; top: 0; z-index: 50`
- `border-b; backdrop-blur-md; transition-all duration-300`
- Scrollé (> 80px) : `border-border bg-background`
- Non scrollé : `border-transparent bg-transparent`

**Container inner :** `mx-auto flex w-full max-w-[1200px] items-center justify-between py-4`

**Logo (Link `href="/"`) :** `flex items-center gap-2 rounded text-left focus-visible:ring-2 focus-visible:ring-blue`
- Texte desktop : `hidden text-lg font-bold text-primary md:inline`
- Texte mobile centré (Link séparé) : `absolute left-1/2 -translate-x-1/2 text-lg font-bold text-primary md:hidden`

**Nav desktop :** `hidden items-center gap-8 text-sm md:flex`
- Items : `text-secondary transition-colors duration-200 hover:text-primary`
- Liens : Analyse → `/analysis` · Prix → `/pricing` · À propos → `/about` · Aide → `/help`

**Bouton auth desktop (conditionnel sur `user`) :**
- `hidden items-center gap-2 rounded bg-blue px-4 py-2 text-sm font-semibold text-primary transition-all duration-200 hover:bg-blue/90 md:inline-flex`
- Icône : `<UserCircle h-4 w-4>`
- Si `user` : texte "Se déconnecter" → `supabase.auth.signOut()`
- Si non connecté : texte **"Se connecter"** si `wasLoggedIn` (`localStorage["atx_was_logged_in"] === "1"`) sinon **"S'inscrire"** → `connectGoogle()` : normalise l'origin (`https://www.alphatradex.ai` → `https://alphatradex.ai`), pose `sessionStorage["oauth_login_pending"] = "1"`, puis `supabase.auth.signInWithOAuth({ provider:"google", redirectTo: \`${origin}/api/auth/callback\`, queryParams:{ access_type:"offline", prompt:"consent" } })`

**Auth state :** géré via `supabase.auth.getSession()` + `onAuthStateChange` dans `useEffect`
- `getSupabaseClient()` → `createBrowserClient` de `@supabase/ssr`
- **`handlePostLogin`** : au retour OAuth (`oauth_login_pending` consommé + événement `SIGNED_IN`/`INITIAL_SESSION`, avec fallback `getSession()` si l'échange a précédé l'enregistrement du listener ; garde anti-double via `redirectingRef`) → lit `users.subscription_status` et redirige `window.location.href` vers `/dashboard` (actif) ou `/pricing`
- Toute session vue pose `localStorage["atx_was_logged_in"] = "1"` (pilote le libellé "Se connecter"/"S'inscrire")

**Burger mobile :** `rounded border border-border bg-card p-2 text-secondary md:hidden`
- Icône : `<Menu>` fermé / `<X>` ouvert

**Menu mobile :**
- `overflow-hidden rounded border border-border bg-card transition-all duration-200 ease-out md:hidden` (pas de `mx-4`)
- Ouvert : `max-h-[360px] opacity-100`
- Fermé : `max-h-0 opacity-0`
- Inner : `flex flex-col gap-2 p-4`
- Liens mobiles : Analyse · Prix · À propos · Aide
- Bouton auth mobile : conditionnel (Déconnexion / S'inscrire), même logique que desktop
- Fermeture au clic extérieur via `mousedown` listener sur `document`

**États / logique :**
- `scrolled` : `window.scrollY > 80`
- `handleBrandClick` : `window.scrollTo({ top:0, behavior:"smooth" })` si sur `/`

---

### `Footer.tsx` — Server Component

```
<footer class="mx-auto flex max-w-[1200px] flex-col gap-8 text-secondary">
  Logo SVG 32x32 + "AlphaTradeX" font-bold text-primary
  "Votre analyste IA sur les marchés" — mt-2 text-sm
  a[href="mailto:contact@alphatradex.ai"] — hover:text-primary
  flex flex-wrap items-center gap-4 text-sm :
    <Info h-4 w-4> Link "/about" → "À propos"
    <HelpCircle h-4 w-4> Link "/help" → "Aide"

<div class="mx-auto mt-6 flex max-w-[1200px] flex-col gap-2 text-xs text-secondary">
  Row 1 : /legal/mentions-legales · /legal/cgu · /legal/confidentialite
  Row 2 : /legal/cookies · /legal/cgv · /legal/risque

<p class="mx-auto mt-8 max-w-[1200px] text-sm text-secondary">
  © 2026 AlphaTradeX. Élaboré pour les traders sérieux.
```

---

### `CookieBanner.tsx` — `"use client"` + `createPortal`

**Décision clé :** `createPortal(…, document.body)` — échappe tout stacking context.

**Module-level flag :** `let _sessionDismissed = false`

**LocalStorage key :** `"alphatradex-cookie-consent"` → `"accepted"` | `"rejected"`

**Styles inline du conteneur :**
```
position: fixed; bottom: 12px; left: 50%
transform: translateX(-50%)
width: min(420px, calc(100% - 2rem))
backgroundColor: #12121A; border: 1px solid #1E2035
borderRadius: 12px; padding: 12px; zIndex: 9999
opacity: exiting ? 0 : 1; transition: opacity 300ms ease-out
```

**Texte :** `fontSize:0.75rem; color:#8892AA; textAlign:center; whiteSpace:nowrap`
"Nous utilisons les [cookies] pour améliorer votre expérience."

**Grille boutons :** `display:grid; gridTemplateColumns:repeat(2,1fr); gap:0.625rem; marginTop:0.625rem`

**Bouton Accepter :** `bg:#2D6FFF; color:#FFF; borderRadius:12px; padding:8px 16px; fontSize:0.75rem; fontWeight:600`

**Bouton Refuser :** `bg:transparent; border:1px solid #1E2035; color:#8892AA; borderRadius:12px; padding:8px 16px; fontSize:0.75rem; fontWeight:600`

**Animation sortie :** `exiting=true` → `opacity:0` → `setTimeout(320ms)` → `show=false`

---

### `Providers.tsx`

```ts
NextTopLoader :
  color:       "#2D6FFF"
  height:      3
  shadow:      "0 0 12px rgba(var(--blue-rgb), 0.35)"
  speed:       250
  showSpinner: false
```

---

### `ScrollReset.tsx`

- Composant minimal : `useLayoutEffect(() => { window.scrollTo(0, 0) }, [pathname])` — scroll en haut à chaque navigation client-side
- `history.scrollRestoration = 'manual'` n'est PAS posé ici : il vient du script inline dans `<head>` (`app/layout.tsx`, cf. §6)
- Retourne `null`

---

### `TradeReportBody.tsx` — contenu pur du rapport (extrait de `TradeReport.tsx`)

`export function TradeReportBody({ report }: { report: AiAnalysisResult })` — composant de présentation pur (sans `"use client"`-only logic, sans animation, sans bouton). Extrait pour être la **source de vérité unique** de la mise en page du rapport, partagée entre :
- `TradeReport.tsx` (UI live, wrapper animé)
- `AnalysisHistory.tsx` (rendu inline dans l'accordéon historique)
- (anciennement aussi `app/api/generate-pdf/route.tsx` via `renderToStaticMarkup` — cette route a depuis été reconstruite en générateur HTML autonome, voir §6 PDF)

**Helpers exportés :** `safeNum(v, fallback=0)`, `safeStr(v, fallback="—")`, `displayRate(v)` (×100 si `v <= 1`, sinon valeur brute, formaté `.toFixed(1)`), `formatSentinelRatio(v)` (sentinelles `99`/`-99` → « ∞ » `text-cyan` / « −∞ » `text-red` pour Profit Factor, Sharpe Ratio et Risk/Reward ; sinon `.toFixed(2)` coloré par seuil ≥ 1). Max Drawdown `null` → « — » `text-secondary`.

**`ScoreCircle` (interne) :** SVG viewBox 100×100, cercle r=45, stroke 8px, normalisation `score*100` si `0 < score <= 1`
- `> 60` → `var(--cyan)` / `>= 40` → `var(--orange)` / `< 40` → `var(--red)`
- Texte centré `font-mono text-sm sm:text-lg font-bold`, label `mt-2 text-xs text-secondary`

**`SeverityBadge` (interne) :**
- CRITIQUE / ÉLEVÉ : `bg-red/20 text-red`
- MOYEN : `bg-orange/20 text-orange`
- FAIBLE : `bg-cyan/20 text-cyan`

**`sessionPctColorClasses(pct)` (interne) :**
- `< 40` → `var(--red)` / `text-red`
- `<= 60` → `var(--orange)` / `text-orange`
- `> 60` → `var(--cyan)` / `text-cyan`

**Sections (`space-y-6`) :**

| # | Titre | Contenu |
|---|---|---|
| 1 | Performance globale | 3 `ScoreCircle` : Score psychologique, Gestion du risque, Prop Firm Readiness — `flex justify-around` |
| 2 | Statistiques clés | `grid grid-cols-2 md:grid-cols-4 gap-4`, cards `rounded-xl bg-hover p-4` : Win Rate, Profit Factor, Max Drawdown (`text-red` si >10% sinon `text-cyan`), PnL Total (`text-red`/`text-cyan`), Trades Total (`text-primary`), Sharpe Ratio, Risk/Reward, Durée moyenne (`text-primary`) |
| 3 | Profil psychologique | `dominantBias` en `text-red`, puis liste de `biases[]` — cards `rounded-xl bg-hover p-4` avec nom, fréquence %, `SeverityBadge`, description, evidence (`text-secondary/70`) |
| 4 | Performance par session | `grid sm:grid-cols-3`, London / New York / Tokyo (fallback `asianWinRate` → `tokyoRate` par défaut 30) — barre `h-3 rounded-full` colorée via `sessionPctColorClasses`, `session.insight` en dessous |
| 5 | Patterns de performance | `grid grid-cols-2 gap-4` : Meilleur/Pire jour, Meilleure/Pire heure (suffixe " UTC" retiré), Meilleur/Pire symbole (`symbole (winRate%)`) — `text-cyan`/`text-red` |
| 6 | Prop Firm Readiness | `estimatedTimeToReady` + liste `mainObstacles[]` avec icône `Check`(vert)/`X`(rouge) selon `wouldPassFTMO`, texte `text-primary`/`text-secondary` |
| 7 | Plan d'action | `actionPlan[]` — cards `rounded-xl bg-hover p-4`, badge catégorie coloré : Psychologie `bg-red/20 text-red`, Risque/Stratégie `bg-orange/20 text-orange`, Timing `bg-cyan/20 text-cyan` |
| 8 | Analyste IA | `personalizedInsight`, `text-sm text-secondary` |

---

### `TradeReport.tsx` — wrapper `"use client"`

`export default function TradeReport({ report, analysesUsed?, analysesLimit? })` — ajoute par-dessus `TradeReportBody` :

1. **Quota bar** (si `analysesUsed !== undefined && !isUnlimited(limit)`, `UNLIMITED = 999999` dans `lib/plans.ts`) : `card flex items-center justify-between p-6` — barre `h-2 w-32 rounded-full bg-hover`, remplissage **toujours `bg-blue`** (pas de variante rouge à saturation), texte `font-mono text-primary` `{used}/{limit}`.
2. **Animation unique** : `motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.4}}` enveloppant `<TradeReportBody report={report} />` (une seule animation globale, plus de delays par section).
3. **Bouton Export PDF** (`handleDownloadPdf`) :
   - `POST /api/generate-pdf` avec `{ report, screenWidth: window.innerWidth < 640 ? window.innerWidth : 1200 }`
   - Réponse → `blob` → `URL.createObjectURL` → `<a download="alphatradex-rapport-{yyyy-mm-dd}.pdf">` cliqué puis retiré, `revokeObjectURL` après 1s
   - États `pdfLoading` (spinner + "Génération en cours...") / `pdfError` (`text-red`)
   - Bouton `btn-primary` avec icône `Download` (lucide), texte "Exporter"

---

### `TradeJournal.tsx` — Journal de trades

Sans props. Affiche l'historique de trades importés (table `trades`) sous forme de calendrier avec vues Jour/Mois, sélecteur de plage de dates et import/export CSV.

**Chargement (`loadTrades`) :**
- `GET /api/trades?token=...` (token = access token Supabase) — **aucun `dateMin` envoyé** : la fenêtre de rétention est appliquée CÔTÉ SERVEUR (`/api/trades`, depuis le plan réel + la date d'inscription, cf. §6) ; le composant ne fait que filtrer l'AFFICHAGE via le sélecteur de dates
- Erreur (`!res.ok` ou exception) → `loadError` ; les réponses d'erreur n'ont pas de corps, le JSON n'est parsé qu'au succès

**Vues (`view: 'day' | 'month'`) :**
- Toggle desktop : texte "Jour · Mois", actif `text-primary font-semibold`, inactif `text-secondary hover:text-primary`
- Toggle mobile : bouton dropdown `"{label} ›"` → panneau `absolute ... bg-card border border-border rounded-xl z-50`, fermeture au clic extérieur

**`DateRangePicker`** (`@radix-ui/react-popover`) :
- Plage par défaut : du 1er jour du mois en cours à aujourd'hui
- Trigger : `text-secondary text-sm hover:text-primary` — affiche `DD/MM/YYYY · DD/MM/YYYY`
- Popover : `background:#12121A; border:1px solid #1E2035; border-radius:12px; padding:16px`
- Header mois : `< {Mois Année FR} >` (`MONTHS_FR`)
- Grille jours `grid-cols-7`, cellules `w-10 h-6 text-xs rounded-xl` :
  - Sélection début/fin : fond `#2D6FFF`, texte blanc
  - Aujourd'hui (non sélectionné) : `text-blue`
  - Plage intermédiaire : `bg-blue/15`
  - Hors mois : `opacity-40 cursor-not-allowed`
- Sélection en 2 clics (`step: 'from' | 'to'`), auto-swap si `to < from`
- Hint : "Clic 1 > date début · Clic 2 > date fin"

**Import / Export :**
- Import : input `.csv` → `detectAndParse` (parseCSV partagé) → `POST /api/import-trades { trades }` → toast succès (message par dominance imported/retention/duplicates, `bg-blue/10 border-blue/30 text-blue`) ou erreur (`bg-red/10 border-red/30 text-red`), auto-dismiss, bouton `X` (lucide)
- Export : `exportCSV()` — CSV français (`Date ouverture, Date fermeture, Symbole, Côté, Entry, Exit, Volume, Profit`), téléchargé `alphatradex-trades-{dateFrom}-{dateTo}.csv`
- Barre desktop : toggle vue (gauche) · `DateRangePicker` (centre) · "Importer · Exporter" (droite, texte)
- Barre mobile : dropdown vue (gauche) · `DateRangePicker` (centre) · icônes `Upload`/`Download` (droite)

**Vue Jour — calendrier mensuel** (`card rounded p-4 mb-6`) :
- Header : `ChevronLeft`/`ChevronRight` mois, bouton "Today", label mois/année
- Grille `grid-cols-8` : 7 colonnes jours (`DAYS_FR = ['Lu','Ma','Me','Je','Ve','Sa','Di']`) + 1 colonne résumé hebdo
- Cellule jour (`min-h-[64px]`) : fond `bg-green/10 border-green/30` si PnL **> 0**, `bg-red/10 border-red/30` si < 0, sinon `bg-card border-border` (**PnL = 0 → fond neutre**, comme un jour sans trades ; le TEXTE PnL, lui, est `text-green` dès ≥ 0) ; sélection `ring-2 ring-blue` ; numéro du jour `text-blue` si aujourd'hui sinon `text-secondary` ; PnL et nb trades `hidden md:block`
- Cellule résumé semaine (8e colonne) : même logique de couleur, PnL total + nb trades + win rate (`hidden md:block`)

**Vue Mois — grille annuelle** (`grid-cols-4 gap-2`, 12 tuiles `min-h-[92px]`) : header avec nav année + "Today" ; mois courant `text-blue`, sinon `text-secondary` ; couleur selon PnL identique à la vue Jour ; `opacity-40` si pas de données

**Panneaux de détail** (`card rounded p-6 mb-6`) :
- Détail jour : liste de trades (`card rounded p-4`) — symbole (`font-bold`), badge côté affichant la **valeur brute `t.side` stockée** (`BUY`/`SELL`, repli `--`) — `bg-green/20 text-green` si `BUY`, `bg-red/20 text-red` sinon (pas de libellés "Long"/"Short"), heures, entry/exit/volume, PnL `text-green`/`text-red`
- Détail semaine / mois : grille `grid-cols-2 md:grid-cols-3 gap-4` — PnL total, Trades, Win rate, Meilleur jour, Pire jour

**Convention couleur PnL :** `text-green` (≥0) / `text-red` (<0) — `formatPnl` → `(pnl>=0?'+':'')+pnl.toFixed(2)+'€'`

**Heures affichées en UTC :** les timestamps DB sont lus via `parseDbDate` (`lib/parseDbDate.ts`) puis affichés avec `toLocaleTimeString('fr-FR', { timeZone: 'UTC' })` — l'heure affichée est l'heure LITTÉRALE stockée, sans dérive d'offset local (cf. §6). Plancher de navigation du calendrier (`minNav`) dérivé de `APP_LAUNCH` (`lib/plans.ts`).

---

### `AnalysisHistory.tsx` — Historique des analyses

Liste les analyses passées de l'utilisateur (`GET /api/analyses`, auth cookie), chacune dépliable vers le `TradeReport` complet.

- État chargement : titre "Historique" + spinner `animate-spin text-secondary`
- État vide : titre "Historique" + overlay `fixed inset-0 top-14 left-0 md:left-[280px]` centré, `pointer-events-none`, "Aucune analyse disponible." (`text-secondary text-sm`)
- Liste : `space-y-3`, items `motion.div` (`initial opacity:0,y:12 → animate opacity:1,y:0`, `delay: i*0.04`), `card overflow-hidden`
- Header item (bouton toggle) : date formatée `DD/MM/YYYY` (`text-secondary`) + PnL `font-mono font-semibold` (**`text-cyan`** si ≥0, `text-red` si <0 — convention différente de `TradeJournal`) + `ChevronUp`/`ChevronDown`
- Expansion : `AnimatePresence` — `height:0→auto, opacity:0→1`, 250ms easeInOut, `border-t border-border`, contenu = `<TradeReport report={a.report} />` (sans props quota)
- Un seul item ouvert à la fois (`expandedId`)

---

## 5. PAGES

> Redesign global (mai-juin 2026) : toutes les pages marketing (`/`, `/pricing`, `/about`, `/help`, `/analysis`) ont été alignées sur un même langage visuel — hero `min-h-screen pt-16 flex items-center justify-center px-6` avec wrapper `mx-auto max-w-[1200px] pb-10 pt-10 md:pb-0 md:pt-0 text-center`, eyebrow `font-mono text-xs uppercase tracking-[0.25em] text-secondary`, H1 `text-5xl font-bold leading-tight text-primary md:text-7xl`, H2 `text-4xl font-bold text-primary md:text-5xl` (`mx-auto mt-4 max-w-[700px]`), sous-titres `max-w-[520px] mx-auto text-base text-secondary`, sections `px-6 py-20` (CTA finales `py-28 text-center`), grilles `grid gap-5 md:grid-cols-{2,3,4}`, et **reveal par élément** : chaque carte/élément d'une grille est enveloppé individuellement dans son propre `<RevealSection delay={i * N}>` (N ∈ {0,40,60,70,80,90,100,140,200,210}) plutôt que la grille entière. Toutes les pages se terminent par une section "Commencer" (eyebrow) + `<RevealSection className="border-t border-border bg-background/80 px-6 py-10 backdrop-blur-md"><Footer /></RevealSection>`.

### `/` — `app/page.tsx` — `"use client"`

Wrapper : `min-h-screen bg-background text-primary` · `<main className="relative overflow-x-clip">`

> **Refonte copy (juillet 2026)** : les textes de la landing sont désormais la **référence produit** (message match — les intitulés des cartes Services reprennent les hooks des plans de `/pricing`). La latence affichée est **"60 secondes"** partout sur le site (plus aucun "moins de 60 secondes" — `/`, `/help`, `/analysis` inclus ; la stat de `/about` affiche "60s"). Alignement : dans les sections Services / Technologie / Système / Témoignages, le wrapper `max-w-[1200px] py-20` n'a plus `text-center` — seuls les en-têtes (`RevealSection className="text-center"`) restent centrés, le **contenu des cartes est aligné à gauche**.

#### Section 1 — Hero
- `RevealSection className="min-h-screen pt-16 flex items-center justify-center px-6 bg-gradient-to-b from-background to-card"`
- Eyebrow : `font-mono text-xs uppercase tracking-[0.25em] text-secondary mb-4` — "AlphaTradeX"
- H1 : `mx-auto mt-6 max-w-[1200px] text-balance md:[text-wrap:normal] text-5xl font-bold leading-tight text-primary md:text-7xl`
  - "Les meilleurs traders n'ont pas plus travaillé.<br/>Ils ont mieux **compris**."
  - "compris" : `<span className="text-blue">` (seul accent bleu de toute la page)
- Séparateur : `mx-auto mt-10 h-px w-12 bg-blue`
- P : `mx-auto mt-8 max-w-[520px] leading-relaxed text-lg text-secondary` — "L'analyste IA décrypte chaque trade et détecte chaque pattern. Vous ne répétez plus deux fois la même erreur."
- **Pas de CTA dans le hero** — les CTA sont reportés dans les sections "Analyse gratuite" et "Commencer"
- **`AccountCreationErrorBanner`** : si `?error=account_creation_failed` (échec de création de la ligne `users` au callback OAuth), une bannière d'erreur s'affiche calée **24px** (`HERO_RHYTHM_GAP`) au-dessus du bloc hero, repositionnée via `ResizeObserver` quand la bannière ou le hero changent de hauteur — même carte rouge standard que le reste du site (cf. §6 « Chemins d'erreur unifiés »)

#### Section 2 — "Tableau de bord" (mockup dashboard, `bg-card`)
- Eyebrow "Tableau de bord" + H2 "Votre espace.<br/>Notre analyse." + sous-titre "Un dashboard pensé pour les traders qui vont exiger la précision."
- `<div ref={dashboardContainerRef} className="card glow-blue rounded overflow-hidden">`, hauteur inline `dashboardHeight`px
- **Scaling viewport** (identique au dashboard Élite réel) :
  - Constantes `DASHBOARD_DESKTOP_WIDTH = 1440`, `DASHBOARD_MOBILE_WIDTH = 390`
  - `updateDashboardScale()` : `isMobileDashboard = window.innerWidth < 768`, `scale = containerWidth / (1440 ou 390)`, `viewportHeight = window.innerHeight`, `dashboardHeight = viewportHeight * scale`
  - Contenu interne : `position:absolute; transform: scale(${scale}); transformOrigin:'top left'`, largeur fixe 1440/390px, hauteur `${viewportHeight}px` — recalculé au `resize`
- Mockup reproduit le dashboard réel : topbar desktop (logo, "AlphaTradeX", point pulsant "IA active", badge "Élite", "Cycle · 01/01", bouton "Se déconnecter" désactivé) + topbar mobile, sidebar desktop (Analyse/Performance/Signaux/Aide/Compte + `ChevronDown`), zone principale "Analyser vos trades" avec drop zone et bouton "Analyser mes trades" désactivé

#### Section 3 — "Compatibilité" (`bg-background`)
- Eyebrow "Compatibilité" + H2 "Vos plateformes.<br/>Notre analyse." + "Votre historique importé. Vos biais exposés."
- `mt-12 flex flex-wrap items-center justify-center gap-5` — 7 pills `card rounded px-7 py-3 font-mono text-sm font-semibold` : MT4 · MT5 · Binance · Bybit · TradingView · FTMO · FundedNext, chacune `<RevealSection delay={index * 60}>`

#### Section 4 — "Services" (`id="services"`, `bg-card`)
- Eyebrow "Services" + H2 "Tout pour trader au plus haut **niveau**." (span `text-primary`) + "Passez de l'instinct à la stratégie." — en-tête centré, cartes alignées à gauche
- `mt-12 grid gap-5 md:grid-cols-3` — `ServiceCard`, **intitulés = hooks des plans de `/pricing`** (message match) :
  - `BrainCircuit` (text-blue) "Structurer ses décisions" (≙ Pro : analyse, métriques, plan d'action) — delay 0
  - `BarChart3` (text-cyan) "Optimiser sa régularité" (≙ Premium : évolution hebdo, verdict, levier) — delay 100
  - `BellRing` (text-green) "Maîtriser son exécution" (≙ Élite : setups, signaux, API) — delay 200

#### Section 5 — "Technologie" (`bg-background`)
- Eyebrow "Technologie" + H2 "Propulsé par **GPT-5.6 Sol**." + "Vous savez lorsque les autres vont deviner. Le modèle IA le plus avancé d'OpenAI sur chaque analyse." — en-tête centré, cartes alignées à gauche
- `mt-12 grid gap-5 md:grid-cols-3` — 3 `article.card.rounded.p-7.hover:border-blue` :
  - `BrainCircuit` (blue) "Précision chirurgicale" ("raisonne en analyste senior") — delay 0
  - `ShieldCheck` (cyan) "Fiabilité inégalée" ("Encore moins d'hallucinations et aucune sur vos données.") — delay 100
  - `TrendingUp` (green) "Disponibilité immédiate" ("Analyse complète générée en 60 secondes.") — delay 200

#### Section 6 — "Système" (`bg-card`)
- Eyebrow "Système" + H2 "Le système que les traders **disciplinés** vont adopter." + "Passez d'un trading émotionnel à une exécution maîtrisée." — en-tête centré, cartes alignées à gauche
- `mt-12 grid gap-5 md:grid-cols-2` — 4 `article.card.rounded.p-7` (titres courts, sans point) :
  - `Trophy` (blue) "Élaboré pour les traders" — delay 0
  - `BarChart3` (cyan) "Fondé sur vos données" — delay 70
  - `BrainCircuit` (green) "Armé contre vos biais" — delay 140
  - `ShieldCheck` (blue) "Adopté en une journée" ("première analyse en 60 secondes") — delay 210

#### Section 7 — "Témoignages" (`bg-background`)
- Eyebrow "Témoignages" + H2 "Ce que les traders **sérieux** ont vu que les autres ont ignoré." + "Tout se trouve dans leur historique. Tout se trouve dans votre historique." — en-tête centré, cartes alignées à gauche
- `mt-12 grid gap-5 md:grid-cols-4` — `article.relative.rounded.border.border-border.bg-card.p-7.hover:border-blue`
  - Guillemet `absolute right-4 top-3 text-5xl font-bold text-blue/20` + étoiles `text-blue` (★★★★★)
  - 4 traders (`delay: index * 70`) : Kilian I. (FTMO) · Lilou O. (TradingView) · Alya L. (Binance) · Nassim S. (MT5) — citations mises à jour (Risk/Reward, overtrading, coupe de profits, session London)

#### Section 8 — "Analyse gratuite" (`id="analyse"`, `bg-card`, `py-28`)
- Eyebrow "Analyse gratuite" + H2 "Ce que l'IA **révèle** en 60 secondes." + "Aucune inscription requise."
- 2 boutons : "Analyse gratuite" (`btn-primary` + `ArrowRight`) → `/analysis` · "S'inscrire" (`btn-outline` + `UserCircle`) → `connectGoogle()`
- Note : "Sans carte bancaire. Analyse immédiate."

#### Section 9 — "Commencer" (hors `<main>`, `bg-gradient-to-b from-card to-background py-28`)
- Eyebrow "Commencer" + H2 "Rejoignez l'**élite** des traders." + "Votre historique. 60 secondes. La vérité."
- 2 boutons : "S'inscrire" (`btn-primary`) → `connectGoogle()` · "Analyse gratuite" (`btn-outline`) → `/analysis`
- Note : "Accès anticipé · 200 places · Prix public à venir · Sans carte bancaire"

#### Footer + bouton retour en haut
- `<RevealSection className="border-t border-border bg-background/80 px-6 py-10 backdrop-blur-md"><Footer /></RevealSection>`
- Bouton fixe `pointer-events-none fixed bottom-6 right-6 z-50`, visible si `scrollY > 300` (`translate-y-0 opacity-100` sinon `translate-y-3 opacity-0`), `<ArrowRight h-5 w-5 -rotate-90>`

---

### `/pricing` — `app/pricing/page.tsx` — `"use client"`

Redesign aligné `/about`/`/help` : même hero (`min-h-screen pt-16 ...`), eyebrow "Prix" (cohérent avec le title du layout "Prix - AlphaTradeX"), H1 "Analysez vos trades.<br/>Améliorez vos performances."

#### Badge accès anticipé
`inline-flex items-center gap-2 rounded-full border border-[#2D6FFF] bg-[#2D6FFF]/10 px-4 py-2 text-sm font-medium text-[#2D6FFF]`
`<Flame h-4 w-4>` + "Accès anticipé · Places limitées" (point médian)
Sous-texte : `text-sm text-[#8892AA]` — "Réservé aux 200 premiers membres."

#### Toggle mensuel/annuel
- Wrapper : `inline-flex items-center rounded-full border border-border bg-card p-1 text-sm` (pas de `mt-8` — l'espacement vient du parent `RevealSection delay={80} className="mt-10 flex flex-col items-center gap-4"`)
- Actif : `rounded-full px-4 py-2 bg-background text-primary` (fond **#0A0A0F**)
- Inactif : `text-[#8892AA]`
- Badge économie : `rounded-full bg-[#2D6FFF] px-2 py-0.5 text-xs font-semibold text-[#F0F4FF]` — "-20%"

#### Checkout — `handleCheckout(planName)` (fonctionnel)
```ts
async function handleCheckout(planName: string) {
  const planKey = planName === "PRO" ? "pro" : planName === "PREMIUM" ? "premium" : "elite"
  const billing = billingMode === "yearly" ? "annual" : "monthly"
  const { data: { session } } = await supabase.auth.getSession()
  if (!session) { router.push('/'); return }
  router.push(`/api/create-checkout?plan=${planKey}&billing=${billing}&token=${session.access_token}`)
}
```

#### Tableau des plans (inchangé, conforme à `lib/plans.ts`)

Les prix affichés sont dérivés de `lib/plans.ts` via `perMonth`/`perYear` (aucune valeur en dur dans la page). Vue mensuelle : prix early + prix public mensuel ("Prix public (à venir) : {normal}" en `font-semibold text-primary`, **NON barré**). **Vue annuelle : prix early mensualisé + total annuel plein BARRÉ (`annualFull`, en €/an, `line-through`) + total annuel réduit (`annualTotal`) + prix public mensualisé (`publicAnnualPerMonth`)** :

| | PRO | PREMIUM | ÉLITE |
|---|---|---|---|
| Hook | "Structurer ses décisions." | "Optimiser sa régularité." | "Maîtriser son exécution." |
| Subhook (sous le hook) | "Analyser ses trades avec méthode." | "Améliorer ses performances avec régularité." | "Exécuter ses décisions avec précision." |
| Mensuel opening | 24.5€/mois | 49.5€/mois | 99.5€/mois |
| Mensuel public (non barré) | 49€/mois | 99€/mois | 199€/mois |
| Annuel opening | 19.6€/mois | 39.6€/mois | 79.6€/mois |
| Annuel barré (`annualFull`) | 470.4€/an | 950.4€/an | 1910.4€/an |
| Annuel réduit (`annualTotal`) | 235.2€/an | 475.2€/an | 955.2€/an |
| Annuel public (`publicAnnualPerMonth`) | 39.2€/mois | 79.2€/mois | 159.2€/mois |
| Badge annuel | -20% | -20% | -20% |
| CTA bg | `bg-[#1E2035]` | `bg-[#2D6FFF]` | `bg-[#1E2035]` |
| Highlighted | non | **oui** | non |

**Carte d'erreur checkout** : si `?error=checkout_failed` (redirection depuis `/api/create-checkout` sur tout échec), la page affiche la carte d'erreur rouge standard via `useSearchParams` (réactif — même pattern que la bannière de la home et la carte du dashboard) ; l'URL est nettoyée par `router.replace` dès l'affichage, avec filet `window.history.replaceState(null, "", "/pricing")` (la carte ne survit pas à un refresh).

#### PREMIUM — wrapper spécial
- `style={{ marginBottom: '-4px' }}` sur le div parent
- Gradient : `linear-gradient(180deg, #2D6FFF 0%, #00E5FF 100%)` `padding: 48px 4px 4px 4px`
- Label : **"Le plus populaire"** — `font-semibold text-white` centré dans le bandeau 48px
- Article inner : `relative overflow-hidden rounded-xl border border-border bg-card p-8` (**p-8**, idem cartes non-highlighted)

#### Bloc prix interne
`mt-6 rounded-lg border border-[#1E2035] bg-[#0A0A0F] p-4`
- Label : `text-xs uppercase tracking-wide text-[#8892AA]` — "Accès anticipé (à vie)"
- Prix principal : `mt-2 text-4xl font-bold text-primary` — suffixe "/mois" en `<span className="text-lg">` (`renderMonthlyPrice`)
- Vue annuelle : ligne `flex items-baseline justify-between` portant le total plein barré `text-sm text-secondary line-through` (`annualFull`) ; en dessous, total réduit `mt-1 text-sm font-semibold text-primary` (`annualTotal`)
- Prix public : `mt-4 text-sm text-[#8892AA]` — "Prix public (à venir) : {normal}" (valeur en `font-semibold text-primary`, non barrée en vue mensuelle)

#### Features par plan (12 features par carte)
```
PRO (4/12 incluses)     : 4 analyses/mois✓, Analyses IA GPT-5.6 Sol✓, Export PDF✓,
                           Journal de vos trades (1 mois)✓,
                           Historique de vos analyses✗, Évolution hebdo✗, Résumé hebdo✗,
                           Support prioritaire✗, Score Prop Firm✗, Détection prédictive✗,
                           Alertes Telegram✗, Accès API✗
PREMIUM (8/12 incluses) : 24 analyses/mois✓, Analyses IA GPT-5.6 Sol✓, Export PDF✓,
                           Journal de vos trades (1 an)✓,
                           Historique de vos analyses (1 an)✓ (libellé via historyLabel →
                           getPlanMonths — 12 mois, cohérent lib/plans.ts),
                           Évolution hebdo✓, Résumé hebdo✓, Support prioritaire✓
                           (cohérent FEATURE_MIN_PLAN.prioritySupport = 'premium'),
                           Score Prop Firm✗, Détection prédictive✗, Alertes Telegram✗, Accès API✗
ÉLITE (12/12 incluses)  : Analyses illimitées, Journal (illimité), Historique (illimité)
                           + TOUTES les features
```

#### CTA dynamique (`renderCTA`)
- **Plan désactivé à la vente (`DISABLED_PLANS = ['elite']`, `lib/plans.ts`)** → bouton Élite neutralisé (désactivé, non cliquable) avec texte **"En cours de mise à niveau"**, apparence d'un bouton actif via `disabled:opacity-100` ; le checkout serveur refuse aussi le plan
- Pas d'abonnement actif (`subscriptionStatus !== 'active' || !currentPlan`) → bouton **"Commencer"** → `handleCheckout(p.name)`
- Plan identique au plan actuel → "Plan actuel"
- Plan supérieur → "Upgrader"
- Plan inférieur → "Downgrader" (hauteur identique aux autres CTA)

#### FAQ (5 questions, ordre actuel)
`<details className="card rounded overflow-hidden">` + `<summary ... p-6>` + corps `px-6 pb-6` — H2 section "Ce que les traders sérieux veulent savoir."
1. Mes données sont-elles en sécurité ?
2. Quels formats sont acceptés ?
3. Comment fonctionne l'accès anticipé ?
4. Puis-je changer ou résilier mon plan ?
5. À qui s'adresse AlphaTradeX ?

#### CTA finale — section "Commencer" standard
`px-6 py-28 text-center` — H2 "Votre mirror.<br/>Sans filtre.", sous-texte "Votre historique. 60 secondes. La vérité.", bouton `<Link href="/analysis" className="btn-primary inline-flex items-center gap-2">Analyse gratuite</Link>` + disclaimer accès anticipé ("Accès anticipé · 200 places · …")

---

### `/analysis` — `app/analysis/page.tsx` — `"use client"`

Redesign "rapport pleine page" aligné `/about`/`/help`/`/pricing`. `SESSION_KEY = 'atx_demo_report'` (sessionStorage) — restaure le rapport au montage (try/catch sur données corrompues).

**State `view: 'analyse' | 'rapport'`** (avec `prevViewRef`) :
- `view === 'rapport'` → `document.documentElement.style.overflow = 'hidden'` + `document.body.style.overflow = 'hidden'` (verrouille le scroll)
- Sortie de `'rapport'` → reset overflow + `window.scrollTo(0,0)`
- `document.body.classList` `"hide-glow"` togglé selon `view === 'rapport'` (ajouté à l'entrée en vue rapport, retiré sinon — masque le glow radial)

#### Section 1 — Hero
- Eyebrow "Analyse" + H1 `text-5xl font-bold leading-[1.1] text-primary md:text-7xl` : "Découvrez ce que l'analyste IA révèle sur un compte de trading."
- Séparateur `h-px w-12 bg-blue` + sous-texte "Votre première analyse gratuite basée sur 120 trades." (le libellé court "Analyse basée sur 120 trades." est celui de la Section 3)

#### Section 2 — "Comment on fonctionne"
- H2 "Trois étapes.<br/>Zéro friction." + sous-texte "Un compte de trading transformé en mirror en 60 secondes." + `grid md:grid-cols-3 gap-5`, steps numérotés **01/02/03** :
  - **01** `Upload` (`#2D6FFF`) "L'historique du compte" — "Un fichier CSV depuis MT4, MT5, Binance, Bybit, TradingView, FTMO, FundedNext. Zéro accès au compte."
  - **02** `Brain` (`#00E5FF`) "L'IA décrypte les trades" — "... résultat en 60 secondes."
  - **03** `FileText` (`#00E5B0`) "Le mirror sans filtre"

#### Section 3 — "Analyse gratuite" (action)
- H2 "Votre première analyse.<br/>Sans inscription." + sous-texte "Analyse basée sur 120 trades."
- Bouton "Découvrir l'analyse" (`handleDemo`, disabled si `loading` UNIQUEMENT — `used` ne désactive pas le bouton, un nouveau clic retombe sur le mur 429) — `POST /api/analyze-demo { trades: demoTrades }`, succès → stocke dans `sessionStorage[SESSION_KEY]` + `view = 'rapport'`, `429` → `used = true`
- Bouton **"Mon analyse"** (affiché seulement si `report` existe) — icône `ScrollText`, `onClick={() => setView('rapport')}` : ré-ouvre la vue rapport pleine page (combiné à la restauration sessionStorage au montage → le rapport survit à un refresh)
- Si `used` : carte info **BLEUE** (`border-blue/30 bg-blue/10 text-blue`) — "L'analyse gratuite a été utilisée. Inscrivez-vous pour analyser vos trades."
- Si `error` : carte rouge standard — "Une erreur est survenue. Relancez l'analyse."
- Si `loading` : barre `motion.div` `width: 0%→90%` sur 8s + "L'IA analyse les trades..."

#### Vue rapport pleine page (overlay, `view === 'rapport'`)
```
position: fixed; inset: 0; zIndex: 50; overflowY: 'auto'; background: var(--background)
└── div.mx-auto.max-w-[1200px].px-6.py-10
    └── <TradeReport report={report} />
```

#### Section 4 — "Commencer" (CTA finale)
- H2 "Votre mirror.<br/>Sans filtre." + bouton "Voir les plans" (`Link` → `/pricing`) + disclaimer accès anticipé

---

### `/dashboard` — `app/dashboard/DashboardClient.tsx` — `"use client"`

Protégé par `proxy.ts` (`@supabase/ssr`, ex-`middleware.ts` — cf. §2 / §6). Redesign complet : **shell app** avec sidebar fixe + topbar, vues commutées en interne (`mainView`) sans navigation de page.

#### Layout général
```tsx
<div className="flex h-screen flex-col overflow-hidden bg-background text-primary">
  <header className="z-20 h-14 shrink-0 border-b border-border bg-card">...</header>
  {mobileMenuOpen && <div className="...md:hidden">...</div>}
  <div className="flex min-h-0 flex-1">
    <aside className="hidden h-full w-[280px] shrink-0 flex-col border-r border-border bg-card md:flex">...</aside>
    <main className="flex min-h-0 flex-1 flex-col bg-[#0A0A0F] p-6 ...">...</main>
  </div>
</div>
```
- App shell plein écran (`h-screen overflow-hidden`), pas de scroll de page — seules certaines zones internes scrollent.
- Sidebar desktop fixe **280px** (`w-[280px]`), masquée sous `md`.
- `app/dashboard/page.tsx` : wrapper `Suspense` (requis pour `useSearchParams`), fallback "Chargement du tableau de bord…".

#### Topbar desktop (`h-14`, `hidden md:flex`)
- **Gauche** : logo SVG 28×28 (identique Navbar/Footer) + "AlphaTradeX" → `/`
- **Centre** : indicateur **"IA active"** — point `h-1.5 w-1.5 animate-pulse rounded-full bg-[#2D6FFF]` + `text-xs text-secondary`
- **Droite** : badge plan `rounded-md border border-[#2D6FFF40] bg-[#2D6FFF15] px-3 py-1 text-xs font-semibold text-blue` (`getPlanLabel(subscriptionPlan)`, **défaut "—"** si plan absent/inconnu — `lib/plans.ts`) · `cycleLabel` "Cycle · DD/MM" (`formatResetDate(analysesResetDate)`, ou "--/--") · bouton "Se déconnecter" (`border border-border ... hover:bg-hover hover:text-primary`)

#### Topbar mobile (`md:hidden`)
- Gauche : logo seul (`aria-label="AlphaTradeX · accueil"`)
- Centre : `<Link href="/">AlphaTradeX</Link>` centré absolu (`absolute left-1/2 max-w-[55%] -translate-x-1/2 truncate`)
- Droite : burger `Menu` (lucide) → toggle `mobileMenuOpen`

#### Menu mobile (panneau sous le header, `md:hidden`)
`flex max-h-[calc(100vh-3.5rem)] flex-col overflow-hidden border-b border-border bg-card` (`#12121A`)
1. Bloc haut : indicateur "IA active" + `cycleLabel` + badge plan, sur une ligne, séparateur `h-px bg-border`
2. Zone scrollable : accordéon de navigation (`renderSidebarAccordion`)
3. Bloc bas : séparateur + bouton "Se déconnecter"
4. `renderQuotaCard()` en bas si applicable
- `useEffect` sur `matchMedia("(min-width: 768px)")` — ferme le menu automatiquement en passant en desktop

#### Sidebar / accordéon de navigation (`renderSidebarAccordion`, partagé desktop+mobile)

`SectionLabel` : bouton toggle par section, `font-mono text-[10px] uppercase tracking-[0.15em] text-secondary`, `ChevronDown` rotate 180° si ouvert. **Toutes les sections sont fermées par défaut** (`openSections`).

| Section | Items (icône — libellé → vue) | Verrouillage |
|---|---|---|
| **Analyse** | `ScanLine` Analyser vos trades → `nouvelle-analyse` | — |
| | `ScrollText` Journal de trades → `journal-analyses` | — |
| | `History` Historique → `historique` | si **Pro** (label "Premium") |
| **Performance** | `TrendingUp` Évolution semaine → `evolution` | si **Pro** ("Premium") |
| | `CalendarCheck` Résumé semaine → `resume-hebdomadaire` | si **Pro** ("Premium") |
| | `Target` Prop Firm Score → `prop-firm-score` | si **non Élite** ("Elite") |
| **Signaux** | `Radar` Détection prédictive → `detection-predictive` | si non Élite |
| | `Bell` Alertes Telegram → `alertes-telegram` | si non Élite |
| | `Webhook` Accès API → `acces-api` | si non Élite |
| **Aide** | `Headphones` Support prioritaire → `support` | si Pro (redirige vers `/help` au lieu de `/pricing`) |
| **Compte** | `CreditCard` Offres → `/pricing` | — |
| | `Receipt` Factures → `<a href="/api/customer-portal?view={mainView}">` (la vue courante est passée en paramètre pour que le retour d'échec préserve le deep-link) | — |

Item actif : `border border-[#2D6FFF25] bg-[#2D6FFF15] text-primary`. Item verrouillé : `opacity-40 cursor-not-allowed` + icône `Lock`, `title="Disponible sur plan {Plan}"`, navigue vers `/pricing` (ou `/help`).

#### Carte quota (`renderQuotaCard`)
Affichée si `hasActiveAccess(subscriptionStatus) && analysesUsed !== undefined && analysesLimit !== undefined && !isUnlimited(analysesLimit)` (exige un abonnement ACTIF en plus du quota fini — masquée pour Élite illimité), en bas de la sidebar/menu mobile :
```tsx
<div className="border-t border-border px-4 pb-5 pt-4" style={{ backgroundColor: '#12121A' }}>
  Analyses utilisées        {analysesUsed}/{analysesLimit}
  <div className="h-1 w-full bg-[#1E2035]"><div className="h-full bg-[#2D6FFF]" style={{ width: `${progressWidth}%` }} /></div>
</div>
```

#### Zone principale (`<main>`)
- Scrollable (`overflow-y-auto`) pour `nouvelle-analyse`, `mon-analyse`, `journal-analyses`, `historique`, `evolution`, `resume-hebdomadaire` et `support` ; sinon `overflow-hidden` (même liste pour le branchement du centrage).

**Vues (`mainView: DashboardView`) :**
| Vue | Contenu |
|---|---|
| `nouvelle-analyse` (défaut) | Drop zone CSV + bouton "Analyser mes trades" (+ "Mon analyse" si un rapport existe en session) |
| `mon-analyse` | `<TradeReport report={analysis} analysesUsed analysesLimit />` ou "Aucun rapport à afficher." |
| `historique` | `<AnalysisHistory />` |
| `journal-analyses` | `<TradeJournal plan={subscriptionPlan} />` |
| `evolution` | `<WeeklyEvolution plan={subscriptionPlan} />` (implémenté — `GET /api/weekly-evolution`) |
| `resume-hebdomadaire` | `<WeeklySummary plan={subscriptionPlan} />` (implémenté — `GET /api/weekly-summary` + cron email) |
| `support` | Vue support prioritaire (mailto support@alphatradex.ai) |
| `prop-firm-score`, `detection-predictive`, `alertes-telegram`, `acces-api` | `<EmptyFeaturePage icon title />` — icône `h-12 w-12 text-secondary opacity-30` + titre `mt-6 text-xl font-bold text-primary` (placeholders, backend non implémenté) |

**Vue "Analyser vos trades" (`nouvelle-analyse`) :**
- H1 centré "Analyser vos trades"
- Drop zone : `border-2 border-dashed rounded-xl p-8 text-center cursor-pointer mb-6` — `border-blue bg-blue/5` (drag) / `border-blue/50 bg-blue/5` (fichier prêt) / `border-border hover:border-blue/50` (idle), `<input accept=".csv">` caché, icône `Upload`, "Importez votre historique de trades" + plateformes
- Boutons (`max-w-md` centré) : "Analyser mes trades" (`btn-primary`, disabled tant que pas de fichier, spinner + "L'IA analyse vos trades..." pendant le chargement) ; si un rapport existe déjà en session, **second bouton "Mon analyse"** (`btn-outline py-2.5`) → `mainView = "mon-analyse"`
- Erreurs : `text-red`, `role="alert"`

#### Synchronisation cross-device des analyses
Au montage, `restoreLatestAnalysis()` charge la dernière analyse (`GET /api/analyses`, tous appareils confondus) et une souscription **Supabase Realtime** (`analyses-insert:${userId}`) met à jour le rapport en direct si une analyse est insérée depuis un autre appareil/onglet.

> Détail (fallback `sessionStorage`, filtre `postgres_changes` sur `public.member_analyses`) : **§6 « Synchronisation cross-device des analyses — Supabase Realtime »**.

#### Deep-link `?view=` et carte d'erreur portail
- **`?view=<DashboardView>`** ouvre directement la vue demandée (ex. bouton « Voir mon espace » de l'email Résumé semaine) — même gating que la sidebar : vue verrouillée pour le plan → paramètre ignoré, on reste sur `nouvelle-analyse` ; tant que le plan n'est pas chargé, tout est verrouillé (l'effet se rejoue quand les booléens de verrouillage changent).
- **`?error=portal_failed`** (redirection depuis `/api/customer-portal` sur échec) : carte d'erreur rouge standard en tête de zone principale ; l'URL est nettoyée par `router.replace` en **préservant `?view=`** (le deep-link reste fonctionnel) — la carte ne survit pas à un refresh.

#### Autres comportements
- `document.body.classList.add("hide-glow")` au montage (retiré au démontage) — désactive le glow radial global sur le dashboard
- `signOut()` → `supabase.auth.signOut()` puis `router.push("/")` + `router.refresh()`
- État chargé via `users` table : `subscription_status, subscription_plan, analyses_used, analyses_limit, analyses_reset_date`

---

### `/about` — `app/about/page.tsx` — `"use client"`

Layout aligné `/pricing`/`/help` (hero `min-h-screen pt-16 ...`). Eyebrow "À propos", H1 `text-5xl font-bold leading-[1.1] text-primary md:text-7xl max-w-[900px]` : "Vous aviez les données.<br/>Personne n'avait encore décrypté vos biais."

| Section | Contenu |
|---|---|
| **Hero** | voir ci-dessus |
| **Le problème** | 4 "truth cards" en grille : edge / exposition / journal / exécution |
| **En chiffres** | 4 stats `grid-cols-2 sm:2 md:4` : `60s` / `5` Plateformes compatibles / `200` traders / `2` Prop Firms — **taxonomie VOULUE (décision propriétaire, pas une incohérence)** : « 5 Plateformes » = plateformes de trading (MT4, MT5, Binance, Bybit, TradingView) et « 2 Prop Firms » = FTMO, FundedNext ; les deux tuiles sont distinctes et totalisent les 7 pills de la section Compatibilité (une prop firm n'est pas une plateforme de trading) |
| **Nos convictions** | 3 principes numérotés **01-03** |
| **Genèse** | timeline 2 colonnes avec ligne verticale : 2022 / 2023 / 2025 / 2026 — entrée 2026 : "AlphaTradeX · Accès anticipé." / "200 places. Phase 0 en cours." |
| **Le fondateur** | "Sacha" + "Trader depuis 2022" — citation, `border-l: 3px solid #2D6FFF` |
| **Compatibilité** | 7 pill-cards : MT4 · MT5 · Binance · Bybit · TradingView · FTMO · FundedNext |
| **CTA final ("Commencer")** | 2 boutons : "Analyse gratuite" (primary) → `/analysis` + "Voir les plans" (outline) → `/pricing` |

H2 sizing uniforme `text-4xl font-bold text-primary md:text-5xl`, reveal par élément (`RevealSection delay={i*N}`), grilles `gap-5`.

---

### `/help` — `app/help/page.tsx` — `"use client"`

Layout aligné `/about`/`/pricing`. Eyebrow "Aide", H1 `text-5xl font-bold leading-[1.1] text-primary md:text-7xl` : "Une question.<br/>Une réponse directe."

| Section | Contenu |
|---|---|
| **Hero** | voir ci-dessus |
| **Comment on fonctionne** | 3 steps numérotés **01-03**, identiques en structure à `/analysis` mais en "vous" : "Exportez votre historique" / "L'IA décrypte vos trades" ("résultat en 60 secondes") / "Votre mirror sans filtre" — sous-texte "Votre historique de trades transformé en mirror en 60 secondes." |
| **Compatibilité** | 7 plateformes en 4+3, instructions d'export par plateforme |
| **FAQ** | 10 questions `<details>/<summary>` (`AccordionItem`, `ChevronDown` rotate 180°) |
| **Support** | Card `MessageCircle`(blue) + `mailto:contact@alphatradex.ai` + "Écrire au support" |
| **CTA final ("Commencer")** | 2 boutons : "Analyse gratuite" + "Voir les plans" |

**Guides d'export par plateforme :**
| Plateforme | Instructions |
|---|---|
| MT4 | Ctrl+T · Account History · Clic droit · Save as Report · CSV |
| MT5 | Ctrl+T · History · Clic droit · Export · CSV |
| Binance | Orders · Trade History · Export Trade History · CSV |
| Bybit | Profile · Account · Data Export · Export Now · Download · CSV |
| TradingView | Panneau broker · Export Data · Balance History · Export · CSV |
| FTMO | Client Area · Metrix · Trading Journal · Export · CSV |
| FundedNext | Via MT4 ou MT5 |

**FAQ (10 questions, ordre actuel) :**
1. Mes données sont-elles en sécurité ?
2. Quels formats sont acceptés ?
3. Puis-je tester avant de m'abonner ?
4. Quand mon compteur d'analyses se remet-il à zéro ?
5. Puis-je changer ou résilier mon plan ?
6. Qu'est-ce que le score Prop Firm Readiness ? (Élite uniquement)
7. L'analyse fonctionne sur tous les types d'actifs ?
8. À qui s'adresse AlphaTradeX ?
9. Mon historique est-il suffisant pour une analyse ? (volume minimum 50 trades)
10. Qu'est-ce que les Alertes Telegram de vos setups ? (Élite uniquement — réponse reformulée : "chaque signal dévoilé sur Telegram", plus de mention "en temps réel")

---

### Pages légales (`/legal/*`)

Layout : `relative min-h-screen bg-background font-sans text-primary z-index:0`
Footer dans : `section.relative.border-t.border-border.bg-background/80.px-6.py-10.backdrop-blur-md`

Structure commune :
- Main : `mx-auto max-w-[800px] px-6 py-20 pt-28`
- H1 : `mb-3 text-3xl font-bold tracking-tight text-primary md:text-4xl`
- Sections : `rounded border border-border bg-card p-6 md:p-8` dans `mt-10 space-y-10`
- H2 : `mb-2 text-xl font-semibold text-primary`
- Texte : `mt-4 text-sm leading-relaxed text-secondary`

Routes : `/legal/mentions-legales` · `/legal/cgu` · `/legal/cgv` · `/legal/confidentialite` · `/legal/cookies` · `/legal/risque`

---

## 6. DÉCISIONS TECHNIQUES

### Tables Supabase (modèle de données — récapitulatif)

Regroupement des tables citées au fil du document (détails de chaque usage dans les sous-sections dédiées) :

| Table | Rôle |
|---|---|
| `users` | Profil + abonnement d'un membre : `subscription_status`, `subscription_plan`, `analyses_used`, `analyses_limit`, `analyses_reset_date`, `stripe_customer_id`, `stripe_subscription_id`, `created_at` (date d'inscription, source de la borne de rétention), `weekly_email_opt_out`. |
| `trades` | Trades importés (Journal) : `opened_at`/`closed_at` (`timestamp without time zone`), `source`, `source_id` (clé de dédup), `side`, entry/exit/volume/profit. Alimente Journal, Évolution et Résumé semaine. |
| `member_analyses` | Historique des analyses membres (`id`, `created_at`, `plan`, `report`, `user_id`) — source de l'Historique et du flux **Supabase Realtime** (INSERT). |
| `visitor_analyses` | Anti-abus démo : 1 analyse par IP (`ip_address`, `used_at`) — **le rapport n'est PAS stocké**. Sert aussi de garde d'accès à `/api/generate-pdf` pour un visiteur. |
| `weekly_email_log` | Idempotence de l'email Résumé semaine (clé `{user_id, week_key}`) — un membre ne reçoit jamais deux fois l'email d'une même semaine. |
| `stripe_webhook_events` | Idempotence du webhook Stripe (`event.id` en clé primaire) — un événement re-livré est acquitté sans être rejoué. |

**RPC** `increment_analyses_used` — incrément **atomique** du compteur `users.analyses_used` (à l'abri des requêtes concurrentes), appelée par `/api/analyze`.

### `overflow-x: clip` sur `<html>` uniquement
```css
html {
  overflow-x: clip;    /* ne crée pas de stacking context (≠ hidden) */
  overflow-y: auto;
}
body {
  overflow-x: visible; /* évite le double scrollport sur iOS */
  overflow-y: visible;
}
```
Raison : deux `overflow-y: auto` créaient deux zones défilantes — scroll bloqué, thumb pleine hauteur, restauration sur le mauvais élément.

### `createPortal` pour CookieBanner
Rendu dans `document.body` → échappe tout stacking context. `z-index: 9999` > Navbar `z-50`.

### Module-level `_sessionDismissed`
Variable hors composant React. Survit aux navigations SPA (remounts) sans rechargement complet.

### Glow radial global (`layout.tsx`)
```
position:fixed; top:50%; left:50%; transform:translate(-50%,-50%)
width:600px; height:600px; border-radius:50%
background: radial-gradient(circle, rgba(45,111,255,0.20) 0%, transparent 70%)
pointer-events:none; z-index:1; aria-hidden
```
Présent sur toutes les pages via Root Layout.
`body.hide-glow .radial-glow { display: none; }` (`globals.css`) — `DashboardClient` ajoute la classe `hide-glow` sur `<body>` au montage (et la retire au démontage) pour désactiver le glow sur le dashboard.

### Mécanisme `.hydrated` — fix flash blanc iOS Safari (remplace l'ancienne règle de transition globale)
```css
html.hydrated *, html.hydrated *::before, html.hydrated *::after {
  transition-property: color, background-color, border-color, box-shadow, opacity;
  transition-duration: 200ms;
  transition-timing-function: ease;
}
html.hydrated, html.hydrated body { transition: none; }
/* prefers-reduced-motion: reduce → transition/animation-duration: 0.01ms !important + scroll-behavior: auto */
```
- `app/layout.tsx` injecte 2 scripts inline pour le mécanisme `.hydrated` :
  1. Dans `<head>`, avant tout paint : `document.documentElement.classList.remove('hydrated')` — bloque toute transition CSS pendant le 1er rendu.
  2. Dans `<body>` : `requestAnimationFrame(() => requestAnimationFrame(() => document.documentElement.classList.add('hydrated')))` — réactive les transitions seulement après le 1er vrai paint (double rAF).
- Raison : sur iOS Safari, les transitions actives pendant l'hydratation provoquaient un flash blanc visible avant l'application du thème sombre. `<html>` a aussi `style={{ backgroundColor: '#0A0A0F', colorScheme: 'dark' }}` inline + `<meta name="theme-color" content="#0A0A0F">` et `<meta name="color-scheme" content="dark">` pour forcer le fond sombre dès le 1er paint, avant tout CSS/JS.
- Un 3ᵉ script inline dans `<head>` pose `history.scrollRestoration = 'manual'`, ce qui **neutralise la restauration de scroll automatique du navigateur** (en complément de `ScrollReset`) — le scroll est piloté manuellement, pas restauré au retour de navigation. (Le layout injecte donc 3 `<script>` inline au total : les 2 du mécanisme `.hydrated` ci-dessus + celui-ci.)

### Génération de PDF — `app/api/generate-pdf/route.tsx` (Puppeteer + `@sparticuz/chromium`)

Remplace l'ancien `window.print()`. Endpoint `POST /api/generate-pdf { report, screenWidth }` (appelé depuis `TradeReport.tsx`), `maxDuration = 60`, `dynamic = 'force-dynamic'`.

- **Garde d'accès (route publique côté proxy, auth propre)** : membre reconnu via cookies de session Supabase ; sinon le visiteur n'est autorisé QUE si son IP figure dans `visitor_analyses` (lecture via `SUPABASE_SERVICE_ROLE_KEY`) — à défaut `401`. Corps limité à `MAX_BODY_BYTES = 1_000_000` (double contrôle `content-length` + longueur réelle du body) → `413`. `screenWidth` clampé [320, 3840] (défaut 1200) ; `page.emulateMediaType('screen')`.

- **HTML auto-suffisant** : `buildHtml()` construit une chaîne HTML complète (`<!DOCTYPE html>...`) avec les variables CSS du thème dupliquées en `:root` (`--background`, `--card`, `--hover`, `--blue`, `--cyan`, `--orange`, `--red`, `--green`, `--primary`, `--secondary`, `--border`), police Inter/JetBrains Mono via Google Fonts, `@page { margin: 0 }`, et `*::-webkit-scrollbar { display: none }` + `scrollbar-width: none` pour masquer les scrollbars dans le rendu.
- **`buildBody()`** : reproduit en HTML brut (chaînes de template, pas de JSX/`renderToStaticMarkup`) les 8 sections de `TradeReportBody.tsx` — réimplémente `safeNum`/`safeStr`/`displayRate`/`scoreCircle`/`severityBadge`/`sessionColors`/icônes Check/X en fonctions locales identiques (mêmes seuils de couleur).
- **Responsive `isMobile`** : `screenWidth < 640` → grilles 2 colonnes / score circles 80px / textes réduits, sinon 4 colonnes / 96px (mêmes breakpoints que `TradeReportBody`).
- **Lancement du navigateur** :
  - Production (`NODE_ENV === 'production'`) : `@sparticuz/chromium` (`chromium.args`, `chromium.executablePath()`) + `puppeteer-core`, `headless: true`, `defaultViewport: { width: viewportWidth, height: 720 }`
  - Dev local : `puppeteer-core` avec `executablePath: process.env.CHROME_PATH || 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe'`, `args: ['--no-sandbox', '--disable-setuid-sandbox']`
- **Rendu** : `page.setContent(html, { waitUntil: 'networkidle0' })` → `page.evaluate(() => document.fonts.ready)` → `contentHeight = max(scrollHeight document/body)` → `page.pdf({ width: '${viewportWidth}px', height: '${contentHeight}px', printBackground: true, margin: 0 })` — un PDF d'une seule page à la taille exacte du contenu.
- Réponse : `Content-Type: application/pdf`, `Content-Disposition: attachment; filename="alphatradex-rapport-DD-MM-YYYY.pdf"`.
- `next.config.mjs` : `serverExternalPackages: ['puppeteer-core', '@sparticuz/chromium']` + `outputFileTracingIncludes: { '/api/generate-pdf': ['./node_modules/@sparticuz/chromium/**/*'] }` (sinon le binaire Chromium n'est pas inclus dans le bundle serverless Vercel).

### Email transactionnel — Resend (`lib/resend.ts`, `lib/emails/confirmationPaiement.ts`)
- Déclenché dans `app/api/webhook/route.ts` sur `checkout.session.completed` (après mise à jour de `users`) — envoie un email de confirmation de paiement via l'API Resend (`RESEND_API_KEY`).
- `lib/emails/confirmationPaiement.ts` génère le HTML de l'email (template autonome, même charte que le PDF : couleurs `--blue`/`--cyan`/dark theme).

### `analyzeTradesDemo` vs `analyzeTradesMember` + `computeStats()` — `lib/openai.ts`

Le générateur d'analyse est désormais scindé en deux chemins :

- **`analyzeTradesDemo(trades, targets?)`** (route `/api/analyze-demo`) : 100% LLM (`gpt-5.6-sol`, temperature par défaut — le modèle rejette toute autre valeur, `max_completion_tokens: 4000`). Le `DEMO_SYSTEM_PROMPT` impose des règles strictes pour générer un rapport JSON complet et crédible à partir de données factices/randomisées : `totalTrades` toujours 120, PnL toujours négatif (-1000€ à -250€), Win Rate 45-65%, scores 25-75 (jamais identiques), sessions London 55-70% / New York 40-55% / Tokyo 25-40% (jamais 0%), sévérité des biais selon `frequency` (FAIBLE 20-30 / MOYEN 35-45 / ÉLEVÉ 50-60 / CRITIQUE 65-75), `dominantBias` et `estimatedTimeToReady` toujours des phrases complètes, tutoiement systématique, variation d'une analyse à l'autre (`RÈGLE VARIATIONS`).
- **`analyzeTradesMember(trades)`** (route `/api/analyze`) : **déterministe** — `computeStats(trades)` calcule toutes les valeurs numériques côté serveur (aucune hallucination possible), puis le LLM (`MEMBER_SYSTEM_PROMPT`, temperature par défaut, `max_completion_tokens: 3000`) ne génère que du **contenu qualitatif** (`sessionInsight`, `dominantBias`, `description`/`evidence` par biais, `consecutiveLossesPattern`, `holdingTimeAnalysis`, `riskIssues`, `mainObstacles`, `estimatedTimeToReady`, `actionPlan`, `personalizedInsight`) à partir du bloc `STATS PRÉCALCULÉES`. La fusion finale associe chaque `biasPatterns[i]` (calculé) à son contenu LLM via `patternKey` ; si le LLM omet un biais, le `name` de repli vient du dictionnaire **`BIAS_LABELS_FR`** (7 clés → libellés UI : `revenge_trading` → "Revenge trading", `direction_bias` → "Biais directionnel", `session_bias` → "Surexposition session", `overtrading` → "Overtrading", `loss_extension` → "Extension des pertes", `confirmation_bias` → "Biais de confirmation", `position_sizing_bias` → "Biais de taille de position" ; clé inconnue → snake_case → espaces).

**Calibrage des appels OpenAI (commit `7cd01cc`, 15/07/2026) — commun aux deux chemins :**
- **`AI_TRADES_CAP = 120`** : l'entrée envoyée à l'IA est bornée à 120 trades. Côté membre : si le fichier dépasse le cap, `aiTrades` = les 120 trades les plus récents (copie triée par `closeTime`, ordre chronologique conservé) — le bloc `TRADES BRUTS` du message user est construit sur `aiTrades` et affiche `tradesSummary.length`, tandis que `computeStats()` reste calculé sur le fichier **ENTIER**. Fichier ≤ 120 → comportement strictement identique. Côté démo, le mélange/troncature (`[...trades].sort(() => Math.random() - 0.5).slice(0, 120)`) vit dans **`app/api/analyze-demo/route.ts`** (pas dans `openai.ts`) — le `slice` plafonne à 120 sans compléter : l'entrée n'est à 120 que parce que le client envoie les 120 `demoTrades`.
- **Client OpenAI : `maxRetries: 0, timeout: 70_000`** — un seul étage de retry (la boucle `callAPI` applicative : 2 tentatives, pause 1s). `maxRetries: 0` coupe les retries internes du SDK (défaut 2) qui empilaient jusqu'à 6 appels HTTP silencieux, chacun borné au timeout défaut du SDK (10 min). Le timeout est de **70s PAR TENTATIVE** ; tentative expirée = `APIConnectionTimeoutError` → catch → la boucle retente.
- **`maxDuration = 150`** sur `/api/analyze` ET `/api/analyze-demo` : 2 tentatives × 70s + 10s d'overhead route — le budget entier est consommé par 2 vraies tentatives.
- **`reasoning_effort: 'low'`** (constante `REASONING_EFFORT` dans chaque fonction) : la tâche est de la rédaction structurée sur stats précalculées, low suffit (medium en repli). Sonde 15/07/2026 : `gpt-5.6-sol` accepte `none/low/medium/high/xhigh`, refuse `minimal`.

**Télémétrie des analyses (commit `ecdd3a4`) — vigie de latence/consommation en production :**
- Succès : `console.log` au format exact
  `[MEMBER ok] duration=<ms>ms attempt=<n> effort=low in=<prompt_tokens> out=<completion_tokens> reasoning=<reasoning_tokens>` (idem `[DEMO ok]`). `duration` = durée TOTALE de l'analyse retries compris (`totalStartedAt`), `attempt` = tentative gagnante, tokens depuis `response.usage` (`?` si absent).
- Échec par tentative : `console.error` `[MEMBER attempt=<n>] ERREUR APPEL OPENAI après <ms>ms` (+ variantes CONTENU VIDE / JSON.PARSE ; côté démo `[DEMO attempt=<n>] tentative échouée/expirée après <ms>ms`) — durée PAR TENTATIVE (`startedAt` local). Payload selon la variante : ERREUR APPEL OPENAI (et le catch démo) logge `message/status/code/type/json` de l'erreur ; CONTENU VIDE logge `{ choices }` ; JSON.PARSE logge `{ message, contentStart, contentEnd }`.
- **Références observées (15/07/2026, effort low, entrée 120 trades)** : tokens d'entrée ~3200 (membre, stats + extrait) vs ~15100 (démo, prompt scénario complet) ; latences réelles membre ~24-45s, démo ~42-69s selon le débit OpenAI (mesures amont dans les commentaires de route : membre 33,5-36,6s, démo 45-51s). **401 transitoires récurrents** côté OpenAI (dont une paire consécutive le 15/07 qui a battu le retry 1s) — les logs `[attempt=]` servent de vigie.

**Règles de rédaction des prompts système (commit `a8df1cb`) :**
- **Communes aux deux prompts** (`DEMO_SYSTEM_PROMPT` et `MEMBER_SYSTEM_PROMPT`) :
  - `RÈGLE VOCABULAIRE INTERNE` — aucun nom technique de champ/variable (`biasPatterns`, `patternKey`, `tokyoWinRate`…) dans les textes rédigés.
  - `RÈGLE VOIX D'ANALYSTE` — rédiger comme si l'analyste avait lui-même analysé l'historique ; jamais "les données fournies", les blocs STATS/TRADES BRUTS, le fichier, le CSV — écrire "tes trades", "ton historique".
  - `RÈGLE DISCRÉTION` — ne jamais se désigner (IA, modèle) ni mentionner le JSON, les champs, le prompt ou les règles.
  - `RÈGLE LANGUE DES DONNÉES` — valeurs anglaises reformulées en français métier ; **London / New York / Tokyo sont des SESSIONS, jamais des lieux** ("la session de Londres" ✓, "à New York" / "London" nu ✗).
  - `RÈGLE DATES LISIBLES` — dates/heures en français lisible ("le 15 juillet vers 13h45"), jamais d'ISO brut ; UTC réservé aux tranches horaires issues des STATS.
- **Membre uniquement** :
  - `RÈGLE SOURCE DES CHIFFRES` — STATS fait foi ; seul `totalTrades` est citable comme total, jamais le nombre d'entrées de TRADES BRUTS (extrait récent possible depuis `AI_TRADES_CAP`), aucun fait chiffré déduit des trades bruts.
  - `RÈGLE PATTERNKEY` — `patternKey` est un identifiant de correspondance : recopié uniquement dans le champ `patternKey`, jamais dans `name`/`description`/`evidence`.
  - `RÈGLE SENTINELLES` — valeurs `N/A`/`null`/sentinelles `99`/`-99` (Profit Factor, Risk/Reward, Sharpe) : ne pas les citer, reformuler qualitativement ou omettre.
- **Démo uniquement** :
  - `RÈGLE NOMENCLATURE DES BIAIS` — les `name` de biais proviennent EXCLUSIVEMENT de la liste fermée des 7 intitulés produit (mêmes libellés que `BIAS_LABELS_FR`) ; "FOMO", "Biais de session" etc. interdits ; description/evidence restent libres.
  - `RÈGLE CHAMPS STRUCTURÉS` — champs de DONNÉES du JSON courts et homogènes (dates "15 juillet 2026", sessions "Londres"/"New York"/"Tokyo", jours "Lundi", aucune phrase) ; textes rédigés avec UN format d'heure unique ("vers 14h30" / "de 09h00 à 11h00 UTC"), heures citées reprises telles quelles des trades.
  - `RÈGLE NOMBRE DE TRADES` — le seul nombre de trades citable dans les textes est 120.
  - `RÈGLE SCÉNARIO` — jamais de référence aux cibles, plages imposées, règles de variation ni au caractère généré de l'analyse.
- Retouche démo : "Utilise des exemples concrets **tirés de son historique**" (remplace "tirés des données fournies", cohérent avec VOIX D'ANALYSTE).

**Grille des minutes de la démo (commit `0b88439`)** — dans `analyzeTradesDemo`, les minutes de `tradesData` sont quantifiées en rotation par index : `DEMO_MINUTES = [0, 2, 5, 7]`, `quantizeMinutes(v, DEMO_MINUTES[i % 4])` sur `openTime` ET `closeTime` (via `toDate`, `setUTCMinutes(minute, 0, 0)` — secondes à zéro, heures inchangées, copie systématique sans mutation du trade d'origine, date invalide retournée telle quelle). Uniquement dans les données ENVOYÉES au modèle, jamais en stockage — aligne les horaires citables sur la RÈGLE CHAMPS STRUCTURÉS. Le chemin membre n'est PAS touché.

**`computeStats()` — formules clés :**
- **Drawdown** : equity curve chronologique (trades triés par `openTime`), `maxDrawdownAbs` = pic-creux max en valeur absolue, `maxDrawdownPct` = ratio creux/pic max. **`maxDrawdownPct` peut être `null`** (pic ≤ 0 avec drawdown, ou ratio > 1) ; les formules de scores et `wouldPassFTMO` utilisent alors `ddForScore = maxDrawdownPct ?? 100` — le null est traité comme risque MAXIMAL, jamais comme 0%.
- **Sharpe ratio** : `(avgPnL / stdDevPnL) * sqrt(n)`.
- **Sessions** : `London` = 07h-14h UTC, `New York` = 14h-00h UTC, `Tokyo` = 00h-07h UTC (basé sur `openTime.getUTCHours()`).
- **Meilleur/pire jour** : Win Rate par jour de semaine, seuil minimum 3 trades (sinon fallback = jour avec le plus de trades).
- **Meilleure/pire heure** : tranches de 2h UTC, seuil minimum 2 trades (sinon fallback = tranche avec le plus de trades, à défaut `'N/A'` — symétrique du fallback jours).
- **Scores** (toujours 0-100, formules pondérées) :
  - `psychoScore` = `winRate*0.4 + min(profitFactor/3,1)*20 + max(0,1-maxDrawdownPct/25)*20 + regularityScore*0.2` (régularité = écart-type du PnL journalier normalisé)
  - `riskScore` = `max(0,1-maxDrawdownPct/15)*40 + min(max(sharpe,0)/2.5,1)*30 + min(riskReward/3,1)*30`
  - `propFirmScore` = `max(0,1-maxDrawdownPct/10)*40 + min(winRate/70,1)*30 + min(profitFactor/2,1)*30`
- **`wouldPassFTMO`** : `maxDrawdownPct < 10 && winRate > 50 && profitFactor > 1 && worstDayLossPct < 5 && totalTrades >= 10` — `worstDayLossPct` = pire PnL journalier en % du PnL brut total (proxy "règle de consistance" FTMO).
- **7 patterns de biais détectés algorithmiquement** (`biasPatterns[]`, `frequency`/`severity` calculés via `severityOf()` : <25 FAIBLE / <50 MOYEN / <75 ÉLEVÉ / ≥75 CRITIQUE) :
  1. `revenge_trading` — trade ouvert dans les 60min après une perte
  2. `direction_bias` — écart Win Rate BUY/SELL > 15%
  3. `session_bias` — pire session avec taux de perte > 55% et exposition ≥ 1% des trades
  4. `overtrading` — jours avec ≥ 2× la médiane de trades/jour ET ≥ 5 trades
  5. `loss_extension` — durée moyenne des pertes > 1.5× durée moyenne des gains
  6. `confirmation_bias` — trade dans le même sens après ≥ 3 gains consécutifs dans ce sens
  7. `position_sizing_bias` — lot moyen des perdants > 1.1× lot moyen des gagnants

### Historique des analyses — table `member_analyses` + rétention par plan

`app/api/analyses/route.ts` (`GET`, `dynamic = 'force-dynamic'`) — accepte soit un cookie de session (`@supabase/ssr`), soit un `?token=` (Bearer, pour les appels cross-tab/Realtime). **Palier Premium+** vérifié via `requirePlanFor('analysesHistory', plan)`. Borne de rétention serveur via `getRetentionFloor(plan, users.created_at)` (`lib/plans.ts`) = la plus récente de `(aujourd'hui − rétention du plan)` et `(date d'inscription)` :

| Plan | Rétention | Accès historique |
|---|---|---|
| `pro` | 1 mois | ❌ refusé (palier Premium+) |
| `premium` | 12 mois (1 an) | ✅ |
| `elite` | illimité (depuis l'inscription) | ✅ |
| sans abonnement / inconnu | 0 | ❌ (statut bloqué par le proxy) |

Retourne `{ analyses: [{ id, created_at, plan, report }] }`, triées par `created_at desc`.

### Rétention & protection serveur — règles centralisées (`lib/plans.ts`)

**Date d'inscription** = `users.created_at` (Postgres `DEFAULT now()`, posée à la création de la ligne dans `auth/callback`, jamais ré-écrite). Source unique de la borne de rétention ; repli `APP_LAUNCH` (2026-01-01) si absente.

**Rétention glissante** (`getRetentionFloor`) — appliquée UNIQUEMENT au **Journal de trades** (`/api/trades`) et à l'**Historique des analyses** (`/api/analyses`), toujours sur le plan ACTUEL. Aucune donnée n'est jamais supprimée à un changement de plan : seule la fenêtre d'affichage change (Pro→Premium ré-élargit immédiatement, Premium→Pro rétrécit sans perte). « Analyser vos trades », « Évolution semaine » et « Résumé semaine » n'appliquent PAS de fenêtre (logiques indépendantes).

**Contrôle de STATUT centralisé** (`proxy.ts`) — `subscription_status === 'active'` est vérifié en amont sur `/dashboard/*` (redirection) et sur toutes les routes `/api/*` (401/403 JSON), SAUF la liste publique (`webhook`, `cron`, `auth`, `create-checkout`, `customer-portal`, `analyze-demo`, `email`, `generate-pdf`). Deny-by-default : toute nouvelle route `/api/*` est gardée automatiquement.

**Contrôle de PALIER par feature** (`FEATURE_MIN_PLAN` + `requirePlanFor`) — table centralisée feature→palier minimum, vérifiée dans chaque route concernée. **FAIL-SAFE** : une feature non déclarée est REFUSÉE par défaut.

> **Pour toute NOUVELLE feature avec route API :**
> 1. Déclarer son palier dans `FEATURE_MIN_PLAN` (`lib/plans.ts`).
> 2. Appeler `requirePlanFor(featureKey, plan)` dans la route.
> 3. Le contrôle de statut est automatique via le proxy étendu.
>
> Oublier (1) **bloque l'accès par sécurité** (fail-safe), ce n'est jamais une faille silencieuse. Si la nouvelle route doit être publique (sans utilisateur actif), l'ajouter à `PUBLIC_API_PREFIXES` dans `proxy.ts`.

### Synchronisation cross-device des analyses — Supabase Realtime

`DashboardClient.tsx` :
1. Au montage, `restoreLatestAnalysis()` appelle `GET /api/analyses?token=...` et affiche `analyses[0]` (la plus récente tous appareils confondus), avec fallback sur `sessionStorage["atx_last_report"]` si l'appel échoue.
2. Souscription **Supabase Realtime** sur le canal `analyses-insert:${userId}` — `postgres_changes` `INSERT` sur `public.member_analyses` filtré `user_id=eq.${userId}` : si une nouvelle analyse est insérée depuis un autre appareil/onglet (ex. analyse lancée sur mobile pendant que le dashboard est ouvert sur desktop), le rapport affiché se met à jour en direct sans rechargement.

### Journal de trades — routes `/api/trades` (GET) et `/api/import-trades` (POST)

**`/api/trades` (GET)** :
- `?token=` **OBLIGATOIRE** (401 sans token) — PAS de fallback cookie, contrairement à `/api/analyses` qui accepte cookie OU token (asymétrie assumée). Auth via `supabase.auth.getUser(token)`.
- Rétention serveur via `getRetentionFloor` ; un `dateMin` client ne peut que RESTREINDRE la fenêtre (`effectiveMin = max(clientMin, floor)`), jamais l'élargir — le client actuel (`TradeJournal`) n'en envoie pas.
- Retourne `{ trades: [...] }` (`select('*')`, tri `opened_at` ascendant). Erreurs en codes nus 401/500. Pas de `requirePlanFor` : le Journal est ouvert à tous les plans actifs.

**`/api/import-trades` (POST)** — `maxDuration = 60`, auth cookies uniquement :
- Réponse : `{ success: true, count, skipped, duplicates }` (le champ est `count`, pas `imported`).
- **Filtre d'inscription** : trades `opened_at < users.created_at` (repli `APP_LAUNCH`) écartés AVANT insert → comptés dans `skipped` ; les trades SANS date sont toujours stockés.
- **Dédup** : `upsert` avec `onConflict: 'user_id,source,source_id'` + `ignoreDuplicates: true` ; `duplicates` déduit du nombre de lignes réellement insérées (`.select('id')`). `source_id` = `computeSourceId(trade)` (`lib/parseCSV/sourceId`).
- **Aucun plafond** de volume ; validation minimale (tableau non vide, sinon 400 nu) ; mapping tolérant au double nommage (`openedAt ?? openTime`, `entry ?? entryPrice`, …). Codes nus 401/400/500.

### Résumé / Évolution semaine — routes, cron email, tables dédiées

- **`GET /api/weekly-evolution` et `GET /api/weekly-summary`** : **double auth `?token=` Bearer OU cookies** (même dualité que `/api/analyses`, non triviale car non partagée par `/api/trades`), palier Premium+ via `requirePlanFor('weeklyEvolution'/'weeklySummary')`, données recalculées à la volée depuis la table `trades` (aucune fenêtre de rétention appliquée).
- **Cron `app/api/cron/weekly-summary-email`** — déclenché par `vercel.json` : `{ "crons": [{ "path": "/api/cron/weekly-summary-email", "schedule": "0 0 * * 1" }] }` (**lundi 00:00 UTC**), auth `Bearer CRON_SECRET`, `maxDuration = 60` (~100 envois max par run).
- **Idempotence** : table **`weekly_email_log`** (clé `{user_id, week_key}`) — un membre ne reçoit jamais deux fois l'email d'une même semaine. **Opt-out** : colonne **`users.weekly_email_opt_out`** (lue par le cron, écrite par `/api/email/unsubscribe`).
- Envoi séquentiel throttlé **600 ms** entre membres, try/catch par membre, en-têtes **`List-Unsubscribe` + `List-Unsubscribe-Post: List-Unsubscribe=One-Click`** (désinscription Gmail un clic) ; réponse JSON `{ok, weekKey, eligible, sent, skipped, failed}`.
- **`/api/email/unsubscribe`** : GET sert des pages HTML autonomes dark-theme (désinscription confirmée / déjà désabonné / réinscription via `?action=resubscribe`) ; POST = one-click (400 si token invalide, 200 sinon). Token HMAC-SHA256 base64url (`lib/emailToken.ts`), comparaison `timingSafeEqual`.

### `lib/parseCSV/utils.ts` — utilitaires de parsing CSV (Journal de trades)

Utilisés par `TradeJournal.tsx` pour l'import CSV (en complément de `parseCSV/index.ts` qui gère le parsing par plateforme pour l'analyse) :
- `cleanSymbol(raw)` : `raw.toUpperCase().replace(/[^A-Z0-9/.]/g, '')` — normalise les symboles (espaces, ponctuation parasite).
- `normalizeCSV(text)` : retire le BOM UTF-8 (`﻿`, fréquent dans les exports MT4/MT5, casse le matching des headers sur Safari mobile) et normalise les fins de ligne (`\r\n`/`\r` → `\n`).
- `detectDelimiter(headerLine)` : compte les `;` vs `,` dans la ligne d'en-tête, retourne le délimiteur majoritaire.
- `makeCSVParser(delimiter)` : retourne `{ line(), num() }` — `line()` split + trim + retire les guillemets ; `num()` gère le séparateur décimal `,`→`.` quand le délimiteur est `;` (format européen).

### `next.config.mjs` — Fallback CI
```js
NEXT_PUBLIC_SUPABASE_URL → placeholder si env absent
NEXT_PUBLIC_SUPABASE_ANON_KEY → JWT placeholder si env absent
```

### `lib/plans.ts` — Prix et IDs Stripe live

Plans identiques à ceux affichés sur `/pricing` (aucune incohérence) :

| Clé | Nom | Limit | Mensuel | Annuel |
|---|---|---|---|---|
| `pro` | Pro | 4 | 24.5€ | 235.2€ |
| `premium` | Premium | 24 | 49.5€ | 475.2€ |
| `elite` | Élite | 999999 | 99.5€ | 955.2€ |

**Plans désactivés à la vente : `DISABLED_PLANS = ['elite']`** (`lib/plans.ts`) — le CTA Élite de `/pricing` est neutralisé (bouton désactivé) et le checkout serveur refuse le plan (`app/api/create-checkout/route.ts`). Pour remettre Élite en vente : le retirer du tableau.

**Price IDs et Product IDs Stripe live :**
| Plan | stripePriceMonthly | stripePriceAnnual | stripeProductId |
|---|---|---|---|
| pro | `price_1To2awCfiBqZlYaUVDueCu2W` | `price_1To2crCfiBqZlYaUuRzJNJam` | `prod_USL2OMHSRxPwPE` |
| premium | `price_1To2flCfiBqZlYaUIVaGGRUJ` | `price_1To2hhCfiBqZlYaUR2QPNhOI` | `prod_USL3p21C1Kc8Rc` |
| elite | `price_1To2kbCfiBqZlYaUqTh42IQJ` | `price_1To2mXCfiBqZlYaUflkYYQVz` | `prod_USL53dmtaWpiDq` |

(L'import `PLANS` de `/analysis` a été retiré — la page ne référence plus les prix.)

### Rate limiting API `/api/analyze`
- `Map<userId, timestamp>` en mémoire — 1 requête / 15s par user. **Le timestamp est posé AVANT toute vérification** : une requête qui échoue ensuite (404/403/400) brûle quand même la fenêtre de 15s ; Map par instance serverless
- `maxDuration = 150` (2 tentatives × 70s + 10s d'overhead route, cf. calibrage `lib/openai.ts`)
- Reset mensuel du quota ancré sur le cycle Stripe : `current_period_start` de l'abonnement (retrouvé via `stripe.customers.search` sur `metadata['userId']`) + `getCycleStart()` (anniversaire mensuel, clamp fin de mois — **fonction locale de la ROUTE `app/api/analyze/route.ts`**, pas de `lib/plans.ts`) — pas de reset au « 1er du mois » ; aucun customer/abonnement actif trouvé côté Stripe → aucun reset tenté ; échec du reset en base = compteur actuel conservé (jamais de quota non persisté)
- **Incrément atomique** du compteur via RPC Supabase **`increment_analyses_used`** (à l'abri des requêtes concurrentes) ; échec d'insert `member_analyses` = fail-soft (l'analyse générée/facturée est quand même renvoyée, erreur loggée)
- **`400` métier AVANT l'appel OpenAI** si aucun trade valide après parsing — le quota n'est pas consommé
- Erreurs en **codes nus** (`new NextResponse(null, { status })` : 401/404/429/400/500) ; seul le `403` quota porte un corps : `{ upgrade: true }` (flag machine — les textes sont décidés côté client)

### Demo `/api/analyze-demo`
- Table `visitor_analyses` Supabase — 1 analyse par IP, insert `{ ip_address, used_at }` **uniquement** (le rapport n'est PAS stocké en base). **IP déjà marquée → mur `429`, PAS de re-livraison** : l'accès au rapport déjà produit passe par le bouton « Mon analyse » côté client (`sessionStorage`, session courante) ; OpenAI n'est JAMAIS rappelé pour une IP marquée.
- Check anti-abus **fail-open** (échec technique du select = console.error + on continue comme si l'IP était inconnue — l'insert reste la barrière anti-abus).
- L'IP n'est brûlée qu'**APRÈS** le succès OpenAI (un échec d'analyse ne consomme pas la démo), mais l'insert doit **réussir AVANT** de rendre le rapport — sinon `500` (aucun rapport rendu sans IP enregistrée) ; conflit `23505` (IP déjà marquée pendant une panne du check, ou requêtes concurrentes) → `429`.
- Entrée du modèle : `[...trades]` mélangés (`sort(() => Math.random() - 0.5)`) puis `.slice(0, 120)`.
- `maxDuration = 150` (2 tentatives × 70s + 10s d'overhead route, cf. calibrage `lib/openai.ts`)
- Paramètres randomisés guidés par cibles calculées
- Erreurs en **codes nus** : `new NextResponse(null, { status })` (400/429/500), chaque étape loggée `[analyze-demo] Échec étape: ...`

### Chemins d'erreur unifiés — « routes en codes nus » + carte rouge standard

Convention transverse (commits `d83d66e`/`b3fad14`) :
- **Routes API appelées en fetch** : les erreurs sont des **statuts HTTP nus** (`new NextResponse(null, { status })`), sans message dans le corps — les textes affichés sont décidés et validés côté client (mapping statut → texte dans chaque composant). Exception : les flags machine (`{ upgrade: true }` sur 403 quota).
- **Routes atteintes par NAVIGATION** (les boutons y naviguent, pas de fetch — toute réponse est vue plein écran) : **jamais de JSON brut**, toujours une redirection vers la page d'origine avec un paramètre d'erreur — `/api/create-checkout` → `/pricing?error=checkout_failed`, `/api/customer-portal` → `/dashboard?error=portal_failed`, `/api/auth/callback` → `/?error=account_creation_failed`.
- **Carte d'erreur rouge standard** (mêmes classes partout) : `rounded-lg border border-red/30 bg-red/10 px-4 py-3 text-center text-sm text-red` — utilisée par `AnalysisHistory`, `TradeJournal`, `WeeklyEvolution`, `WeeklySummary`, `DashboardClient` (dont carte `portal_failed`), la carte checkout de `/pricing` et la bannière `account_creation_failed` de la home.

### Heures du Journal en UTC — `lib/parseDbDate.ts`

Point d'entrée UNIQUE de **lecture** des timestamps trades venus de la DB (pendant de `parseTradeDate` qui couvre l'écriture). Les colonnes `opened_at`/`closed_at` sont des `timestamp without time zone` : PostgREST les renvoie SANS offset, et un `new Date()` nu les interprète en heure LOCALE (dérive selon le navigateur/l'hôte). `parseDbDate(raw)` ajoute `'Z'` quand aucun offset n'est porté (regex `Z|[+-]\d{2}:?\d{2}$`) ; une chaîne déjà datée passe telle quelle. `TradeJournal` affiche ensuite les heures via `toLocaleTimeString('fr-FR', { timeZone: 'UTC' })` — l'heure affichée = l'heure LITTÉRALE stockée. Migration du schéma vers `timestamptz` = backlog assumé.

### @supabase/ssr — Migration terminée

Tous les fichiers utilisent `@supabase/ssr`. `@supabase/auth-helpers-nextjs` a été retiré de `package.json` (n'était plus utilisé dans le code applicatif).

| Fichier | Fonction exportée | Client utilisé |
|---|---|---|
| `lib/supabase.ts` | `getSupabaseClient()` | `createBrowserClient` (@supabase/ssr) |
| `lib/supabase.ts` | `getSupabase()` | `createClient` (@supabase/supabase-js) — **CODE MORT, aucun appelant** (create-checkout instancie `createClient` directement ; cf. §8 dette) |
| `lib/supabase.ts` | `getSupabaseAdmin()` | `createClient` avec `SUPABASE_SERVICE_ROLE_KEY` — utilisé dans webhook |
| `proxy.ts` | — | `createServerClient` (@supabase/ssr) |
| `app/api/auth/callback/route.ts` | — | `createServerClient` (@supabase/ssr) |
| `app/api/customer-portal/route.ts` | — | `createServerClient` (@supabase/ssr) |
| `components/Navbar.tsx` | — | `getSupabaseClient()` |
| `app/dashboard/DashboardClient.tsx` | — | `getSupabaseClient()` |

### `parseCSV/index.ts` — Détection automatique de format

```
firstLine contient "position" + "fee"  → parseFTMO  (FTMO Metrix — extra colonne Fee décale Swap/Profit)
firstLine contient "ticket" ou "position" → parseMT4  (MT4 / MT5 standard)
firstLine contient "pair", "realized profit" ou "date(utc)" → parseBinance
firstLine contient "realized p&l"          → parseBybit
firstLine contient "trade #", "signal" ou "cum. profit" → parseTradingView
Sinon → Error "Votre fichier n'a pas été reconnu. Vérifiez qu'il provient d'une plateforme compatible."
        (message unique, affiché tel quel sur les deux surfaces membres — la liste des
         plateformes vit sur la dropzone et /help, pas dans l'erreur)
```

**Plateformes supportées :** MT4, MT5, Binance, Bybit, TradingView, FTMO, FundedNext (7 plateformes)

> **FundedNext** : couvert via l'export MT4/MT5 (le membre exporte son historique depuis la
> plateforme MT4/MT5 de FundedNext → `parseMT4`). Pas de parseur dédié — décision propriétaire.

**`parseFTMO` :** colonnes spécifiques — `Commission[10] + Fee[11]` combinés, `Swap[12]`, `Profit[13]` (décalage vs MT5 standard).

**`parseBybit` :** format "Trade History" — Date, Symbol, Side, Price, Quantity, Fee, Realized P&L, Order ID. Session calculée par heure UTC.

---

## 7. CONFIGURATION

### Variables d'environnement référencées dans le code

| Variable | Utilisée dans |
|---|---|
| `NEXT_PUBLIC_SUPABASE_URL` | ~15 fichiers : `lib/supabase.ts`, `proxy.ts`, `next.config.mjs`, la quasi-totalité des routes `api/*`, et 3 composants client (`TradeJournal.tsx`, `WeeklySummary.tsx`, `WeeklyEvolution.tsx` — `createBrowserClient` en import dynamique) |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Idem |
| `SUPABASE_SERVICE_ROLE_KEY` | `lib/supabase.ts` → `getSupabaseAdmin()` → `api/webhook` ; aussi en direct dans `api/analyze-demo`, `api/generate-pdf`, `api/cron/weekly-summary-email`, `api/email/unsubscribe` |
| `STRIPE_SECRET_KEY` | `api/create-checkout`, `api/customer-portal`, `api/webhook`, `api/analyze` (reset de cycle via `stripe.customers.search`) |
| `STRIPE_WEBHOOK_SECRET` | `api/webhook` (validation signature) |
| `NEXT_PUBLIC_APP_URL` | `api/create-checkout` (success/cancel URL), `api/customer-portal` (return_url), `api/auth/callback` (redirections), `api/cron/weekly-summary-email`, `api/email/unsubscribe`, `app/sitemap.ts`, `app/robots.ts` |
| `OPENAI_API_KEY` | `lib/openai.ts` |
| `RESEND_API_KEY` | `lib/resend.ts` → email de confirmation de paiement (`api/webhook`) ET email Résumé semaine (`api/cron/weekly-summary-email`) |
| `RESEND_FROM` | `api/cron/weekly-summary-email` — adresse d'expéditeur de l'email hebdo (défaut `AlphaTradeX <contact@alphatradex.ai>`) |
| `CRON_SECRET` | `api/cron/weekly-summary-email` (auth Bearer du cron Vercel) ; repli temporaire de `lib/emailToken.ts` |
| `UNSUBSCRIBE_SECRET` | `lib/emailToken.ts` — signature HMAC des liens de désinscription email (`api/email/unsubscribe`) |
| `CHROME_PATH` | `api/generate-pdf` (dev local uniquement) — chemin de l'exécutable Chrome pour Puppeteer ; défaut `C:\Program Files\Google\Chrome\Application\chrome.exe` |

### `next.config.mjs`
```js
const nextConfig = {
  serverExternalPackages: ['puppeteer-core', '@sparticuz/chromium'],
  outputFileTracingIncludes: {
    '/api/generate-pdf': ['./node_modules/@sparticuz/chromium/**/*'],
  },
  env: {
    NEXT_PUBLIC_SUPABASE_URL:
      (process.env.NEXT_PUBLIC_SUPABASE_URL || "").trim() || placeholderUrl,
    NEXT_PUBLIC_SUPABASE_ANON_KEY:
      (process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || "").trim() || placeholderAnon,
  },
};
```
`serverExternalPackages` + `outputFileTracingIncludes` sont nécessaires pour que le binaire Chromium de `@sparticuz/chromium` soit inclus dans le bundle serverless de `/api/generate-pdf` sur Vercel.

### `vercel.json`
```json
{ "crons": [{ "path": "/api/cron/weekly-summary-email", "schedule": "0 0 * * 1" }] }
```
Cron Vercel du Résumé semaine — **lundi 00:00 UTC** (cf. §6 « Résumé / Évolution semaine »).

### SEO — `app/robots.ts` / `app/sitemap.ts` / images OG
- `robots.ts` : `disallow: ['/dashboard', '/api/', '/auth']` ; sitemap `${NEXT_PUBLIC_APP_URL ?? 'https://alphatradex.ai'}/sitemap.xml`
- `sitemap.ts` : 11 URLs — `/` (weekly, priorité 1), `/pricing` et `/analysis` (monthly, 0.7), `/help` et `/about` (monthly, 0.5), les 6 pages `/legal/*` (yearly, 0.2) ; base `NEXT_PUBLIC_APP_URL ?? 'https://alphatradex.ai'`
- `opengraph-image.tsx` : image OG dynamique 1200×630 (`next/og` `ImageResponse`), alt "AlphaTradeX - Votre analyste IA sur les marchés", police Inter fetchée sur Google Fonts avec repli build-safe ; `twitter-image.tsx` = simple ré-export de `opengraph-image.tsx`

### `app/layout.tsx`
```tsx
// Fonts :
Inter({ subsets:["latin"], weight:["400","500","600","700"], variable:"--font-inter" })
JetBrains_Mono({ subsets:["latin"], weight:["400","500"], variable:"--font-jetbrains-mono" })

// Metadata :
metadataBase: 'https://alphatradex.ai'
title: "AlphaTradeX - Votre analyste IA sur les marchés"
description: "Votre analyste IA décrypte vos trades, mesure votre évolution, affûte vos exécutions."
openGraph: { title, description, type:"website", locale:"fr_FR" }
twitter: { card:"summary_large_image", title, description }
// images OG/Twitter : convention de fichiers Next.js (app/opengraph-image.tsx, app/twitter-image.tsx)
// icons : plus de propriété explicite — détection automatique Next.js
// via convention de fichiers (app/icon.svg, app/apple-icon.svg)

// Arbre :
<html lang="fr" data-scroll-behavior="smooth" className={fontVars} style={{ backgroundColor:'#0A0A0F', colorScheme:'dark' }}>
  <head>
    <meta name="theme-color" content="#0A0A0F"/>
    <meta name="color-scheme" content="dark"/>
    <link rel="manifest" href="/manifest.json"/>
    <script>{/* history.scrollRestoration = 'manual' */}</script>
    <script>{/* document.documentElement.classList.remove('hydrated') — anti flash blanc */}</script>
  </head>
  <body style={{ backgroundColor:'#0A0A0F' }}>
    <script>{/* double rAF → classList.add('hydrated') */}</script>
    <div aria-hidden className="radial-glow" />   {/* glow radial fixed z:1, masqué via body.hide-glow */}
    <ScrollReset />
    <Providers>                {/* NextTopLoader */}
      {children}
    </Providers>
    <CookieBanner />           {/* portal → body, z:9999 */}
  </body>
</html>
```

### `proxy.ts` — `@supabase/ssr` (ex-`middleware.ts`, convention Next.js 16)
```ts
import { createServerClient } from '@supabase/ssr'
// Garde de STATUT centralisée : /dashboard/* (redirections) + /api/* (401/403 JSON),
// sauf PUBLIC_API_PREFIXES. createServerClient avec cookies.getAll / setAll.
export const config = { matcher: ['/dashboard/:path*', '/api/:path*'] }
```

### `app/api/auth/callback/route.ts` — `@supabase/ssr`
```ts
// GET handler — reçoit ?code=...
// createServerClient avec cookieStore
// supabase.auth.exchangeCodeForSession(code) — échec capturé : console.error
//   + redirect /?error=account_creation_failed SANS poser les cookies de session
// Échec de l'INSERT users (code ≠ 23505, « compte fantôme ») → MÊME redirection
//   /?error=account_creation_failed sans cookies ; 23505 (ligne déjà existante) → on continue
// Pas de ?code → redirect NEXT_PUBLIC_APP_URL nu
// Succès → update profil si ligne existante / insert sinon (created_at = DEFAULT now()),
//   puis redirect /dashboard ou /pricing selon hasActiveAccess(statut)
```

### Stripe — `app/api/create-checkout/route.ts`
```
GET /api/create-checkout?plan=pro|premium|elite&billing=monthly|annual&token={access_token}

0. AUTH — identité PORTÉE par les requêtes (fix RLS « 0 rows ») :
   - token présent → createClient anonyme AVEC header Authorization: Bearer {token}
     sur toutes les requêtes (l'ancien client nu validait le JWT mais auth.uid()
     restait NULL sous RLS → tous les clics de plan échouaient en PGRST116)
   - sinon repli cookies (createServerClient, navigation same-origin)
   - non connecté → redirect /
1. Plan inconnu OU plan ∈ DISABLED_PLANS → échec (bloqué AVANT tout appel
   Stripe, sur les DEUX chemins : nouvel abonnement ET modification)
2. Lecture users (stripe_subscription_id, subscription_status) en maybeSingle :
   ligne absente = état « pas d'abonnement » ; ERREUR DB → échec (traiter une
   erreur comme « pas d'abonnement » créerait un SECOND abonnement)
3. Si abonnement actif → stripe.subscriptions.update() (upgrade/downgrade,
   proration_behavior:'none', metadata { planName, analysesLimit })
   puis redirect /dashboard?updated=true — jamais de création de customer
4. Sinon → stripe.customers.create({ email, metadata:{ userId } })
         + stripe.checkout.sessions.create({ mode:'subscription', ... })
   puis redirect vers session.url (Stripe Checkout)

TOUT échec → redirect /pricing?error=checkout_failed (jamais de JSON brut —
la route est atteinte par navigation, cf. « Chemins d'erreur unifiés »).
Backlog connu : passer le flux en cookies seuls et retirer le ?token= de l'URL.

Metadata Stripe checkout : { userId, planName, analysesLimit }
success_url : NEXT_PUBLIC_APP_URL/dashboard?success=true
cancel_url  : NEXT_PUBLIC_APP_URL/pricing
```

### Stripe — `app/api/webhook/route.ts`

**Idempotence — table `stripe_webhook_events`** : Stripe peut livrer un même événement plusieurs fois. `event.id` est inséré dans la table AVANT traitement ; un doublon (conflit clé primaire, code `23505`) est acquitté immédiatement (`{ received: true, duplicate: true }`) sans rien rejouer. Table de dédup indisponible → on traite quand même (la facturation prime sur la déduplication). **`failEvent()`** : tout échec d'écriture de l'état d'abonnement retourne `500` pour que Stripe re-livre l'événement, en retirant d'abord `event.id` de la dédup (sinon la re-livraison serait acquittée comme doublon sans être rejouée).

**Événements traités :**

| Événement | Action |
|---|---|
| `checkout.session.completed` | Met à jour `users` : `subscription_status=active`, `subscription_plan`, `analyses_limit`, `analyses_used=0`, `analyses_reset_date` (= `current_period_start` de l'abonnement Stripe — même ancre que `getCycleStart` dans `/api/analyze`), `stripe_customer_id`, `stripe_subscription_id`. Met aussi à jour `sub.metadata` pour les handlers suivants. |
| `customer.subscription.updated` | Upgrade (`newLimit > dbLimit`) : met à jour plan et limit immédiatement. Downgrade : différé au renouvellement de période. `cancel_at_period_end → true` : garde `status=active`, attend `.deleted`. |
| `customer.subscription.deleted` | `status=canceled`, `subscription_plan=null`, `analyses_limit=0` (compte sans abonnement = 0 analyse, accès coupé). Résolution user via `customer.metadata.userId` ou `customer.email`. |

> **Échec de paiement (approche « Stripe pilote »).** Aucun handler dédié : Stripe gère ses relances (`past_due`), l'utilisateur conserve l'accès pendant cette période de grâce, puis l'accès est coupé à la résiliation effective (`customer.subscription.deleted`). **Prérequis Dashboard Stripe** : *Settings → Billing → Subscriptions → « If all retries fail » = « Cancel subscription »*, sinon l'abonnement ne sera jamais supprimé et l'accès jamais coupé.

> **Source de vérité des plans : `lib/plans.ts`.** Toutes les valeurs (limites, fenêtres d'historique, seuil illimité `UNLIMITED`, libellés, éligibilité) y sont définies une seule fois et exposées via des helpers (`getPlanLimit`, `getPlanMonths`, `isUnlimited`, `hasActiveAccess`, `isWeeklyEmailEligible`, `getPlanLabel`). Fenêtres d'historique glissantes : Pro = 1 mois, Premium = 12 mois (1 an), Élite = illimité. La valeur `'starter'` a été éliminée.

### Garde d'accès — `proxy.ts` (ex-`middleware.ts`)

Next.js 16 : la convention `middleware` est dépréciée et renommée `proxy`. Le fichier `proxy.ts` (export nommé `proxy`, `config.matcher = ['/dashboard/:path*', '/api/:path*']`) centralise le **contrôle de STATUT** (`subscription_status === 'active'`, via `hasActiveAccess`) :
- **Pages** `/dashboard/*` : non connecté → redirection `/` ; connecté sans abonnement actif (résilié / impayé / jamais abonné) → redirection `/pricing` ; actif → accès.
- **Routes** `/api/*` : non connecté → `401` JSON ; sans abonnement actif → `403` JSON (`upgrade: true`) ; actif → accès. **Deny-by-default** — toute nouvelle route `/api/*` est gardée automatiquement.
- **FAIL-OPEN sur erreur technique Supabase** : si la lecture de `subscription_status` échoue (panne passagère, timeout — `maybeSingle` réserve `error` aux vraies pannes, ligne absente = refus légitime), le proxy **laisse passer** avec `console.error` plutôt que d'éjecter un abonné actif sur un incident transitoire. Fenêtre résiduelle assumée : les routes derrière re-vérifient l'auth et le palier/les limites du plan (la résiliation met `analyses_limit` à 0, donc aucune analyse consommable sur cette fenêtre).
- **Exclusions publiques** (`PUBLIC_API_PREFIXES`) : `webhook`, `cron`, `auth`, `create-checkout`, `customer-portal`, `analyze-demo`, `email`, `generate-pdf` (auth propre ou aucune donnée membre lue). Le webhook Stripe et le cron ne sont jamais bloqués.

Le **contrôle de PALIER** (Premium/Élite) n'est PAS fait ici : chaque route le vérifie via `requirePlanFor` (cf. « Rétention & protection serveur — règles centralisées »).

### Stripe — `app/api/customer-portal/route.ts`
```
GET /api/customer-portal

1. Récupère l'user (@supabase/ssr, cookies)
2. Récupère stripe_customer_id depuis table users — une ERREUR DB de lecture
   (distincte de « pas de customer ») → portalFailed()
3. Si pas de stripe_customer_id → redirect /pricing
4. stripe.billingPortal.sessions.create({ customer, return_url: APP_URL/dashboard })
5. redirect → session.url (Portail Stripe natif)

Échec → portalFailed() : redirect /dashboard?view={vue courante}&error=portal_failed —
le paramètre ?view= de la requête entrante est recopié dans la redirection pour
préserver le deep-link (cf. lien « Factures » du dashboard).
```

### `public/manifest.json`
```json
{
  "name": "AlphaTradeX",
  "short_name": "AlphaTradeX",
  "icons": [{ "src": "/logo.svg", "sizes": "any", "type": "image/svg+xml" }],
  "theme_color": "#0A0A0F",
  "background_color": "#0A0A0F",
  "display": "standalone"
}
```

---

## 8. ÉTAT D'AVANCEMENT ET RESTE À FAIRE

### Fait depuis la précédente version de ce résumé (livré avec ce document au 20/07/2026 — commits `ecdd3a4` → `236b64d`)
- ✅ **Analyses calibrées** (`7cd01cc`) : `AI_TRADES_CAP` 120, `maxRetries: 0` + timeout 70s/tentative, `reasoning_effort: 'low'`, `maxDuration = 150` sur les deux routes d'analyse
- ✅ **Télémétrie des analyses** (`ecdd3a4`) : logs `[MEMBER ok]`/`[DEMO ok]` (duration/attempt/effort/in/out/reasoning) + durées par tentative sur les erreurs — vigie latence/tokens/401 transitoires
- ✅ **Règles de rédaction des prompts IA** (`a8df1cb`) : règles communes (VOCABULAIRE INTERNE, VOIX D'ANALYSTE, DISCRÉTION, LANGUE DES DONNÉES, DATES LISIBLES), membre (SOURCE DES CHIFFRES, PATTERNKEY, SENTINELLES), démo (NOMENCLATURE DES BIAIS, CHAMPS STRUCTURÉS, NOMBRE DE TRADES, SCÉNARIO) + dictionnaire `BIAS_LABELS_FR`
- ✅ **Grille des minutes de la démo** (`0b88439`) : minutes quantifiées 00/02/05/07 en rotation par index dans `analyzeTradesDemo` (données envoyées au modèle uniquement)
- ✅ **Refonte copy landing** (`a8df1cb`/`959b9ac`/`236b64d`) : textes = référence produit (message match Services↔plans), latence "60 secondes" partout (plus de "moins de 60"), cartes Services/Technologie/Système/Témoignages alignées à gauche
- ✅ **Nettoyage composants morts** : `components/GoogleAuthButton.tsx` et `components/UploadZone.tsx` supprimés (non utilisés)
- ✅ **`@supabase/auth-helpers-nextjs`** retiré de `package.json` (migration `@supabase/ssr` terminée, dépendance inutile)
- ✅ **Branding favicon/icônes** : `app/favicon.ico` supprimé, `app/icon.svg` remplacé par le vrai logo AlphaTradeX (identique à `public/logo.svg`), `app/apple-icon.svg` créé (même contenu que `public/logo.svg`) — détection automatique Next.js via convention de fichiers
- ✅ **Metadata `app/layout.tsx`** : propriété `icons` retirée (gérée par convention de fichiers)
- ✅ **Nettoyage `public/`** : `file.svg`, `globe.svg`, `next.svg`, `vercel.svg`, `window.svg` supprimés (assets par défaut Next.js inutilisés)
- ✅ **`package-lock.json`** resynchronisé via `npm install`
- ✅ **Paiement live testé + remboursement immédiat validé**
- ✅ **Redirections email Google Workspace configurées**
- ✅ **Menu mobile navbar largeur corrigée**
- ✅ **Redesign complet pages marketing aligné design system**

### Précédemment
- ✅ **Export PDF réel** : remplacé par `app/api/generate-pdf/route.tsx` (Puppeteer-core + `@sparticuz/chromium`, HTML auto-suffisant, PDF 1 page à la taille du contenu)
- ✅ **Historique des analyses** : `<AnalysisHistory />` (vue `historique` du dashboard) consomme `GET /api/analyses`, rétention par plan (`getPlanMonths`, `lib/plans.ts`)
- ✅ **Journal de trades** : `<TradeJournal />` (vues calendrier jour/mois, import/export CSV, date range picker)
- ✅ **Synchronisation cross-device** : Supabase Realtime (`analyses-insert:${userId}`)
- ✅ **Email de confirmation de paiement** : Resend, déclenché sur `checkout.session.completed`

### Priorité 2 — Features core manquantes (backend absent)
Présentes dans la sidebar du dashboard sous forme de `<EmptyFeaturePage>` (placeholders verrouillés selon le plan), mais sans implémentation backend :
- **Alertes Telegram** (Élite)
- **Détection prédictive de setups** (Élite)
- **Accès API** (Élite)
- **Prop Firm Score** (Élite — distinct du `propFirmScore` déjà calculé dans le rapport d'analyse)

Implémentées depuis :
- ✅ **Évolution semaine** / **Résumé semaine** (Premium/Élite) — `<WeeklyEvolution />` / `<WeeklySummary />`, routes `GET /api/weekly-evolution` et `GET /api/weekly-summary`, libs `lib/weeklyData.ts` / `lib/weeklySummary.ts` / `lib/weeklyFormat.ts`, cron email `api/cron/weekly-summary-email` (+ désinscription `api/email/unsubscribe`)
- ✅ **Support prioritaire** (Premium) — vue dédiée du dashboard (mailto support@alphatradex.ai)

### Priorité 3 — Finitions
- ✅ résolu — **Page 404 personnalisée** (`app/not-found.tsx`, cf. §2)
- ✅ résolu — **SEO** : sitemap, robots.txt, balises meta OpenGraph et Twitter Card (`app/sitemap.ts`, `app/robots.ts`, `app/opengraph-image.tsx`, `app/twitter-image.tsx` — cf. §7)
- ✅ résolu — `app/icon.svg` vs `public/logo.svg` : cohérence favicon
- ✅ résolu — `@supabase/auth-helpers-nextjs` : retiré de `package.json`

### Dette assumée (constatée à l'audit du 20/07/2026 — code NON corrigé, documenté ici)
- **`getSupabase()`** (`lib/supabase.ts`) : code mort — aucun appelant dans le repo (create-checkout instancie `createClient` de `@supabase/supabase-js` directement)
- **Prop `plan` morte** dans `WeeklyEvolution.tsx` et `WeeklySummary.tsx` : reçue mais jamais utilisée (aucun gating client — le palier Premium+ est vérifié côté serveur par les routes)
- **`README.md`** : boilerplate create-next-app intact (mentionne encore la font Geist, non utilisée)
- **`emails-preview/confirmation.html`** : artefact de preview de l'email de confirmation, non branché (à garder comme fixture ou supprimer)

