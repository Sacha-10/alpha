# RÉSUMÉ COMPLET — AlphaTradeX (à jour au 8 juillet 2026)
...
- **Plan désactivé à la vente (`DISABLED_PLANS = ['elite']`, `lib/plans.ts`)** → bouton Élite neutralisé (désactivé, non cliquable) ; le checkout serveur refuse aussi le plan
...
- **Supabase Realtime** : canal `analyses-insert:${userId}` sur `postgres_changes` `INSERT` de `public.member_analyses` (`user_id=eq.${userId}`) — met à jour `analysis` en direct si une nouvelle analyse est insérée depuis un autre appareil/onglet.
...
- Reset mensuel du quota ancré sur le cycle Stripe : `current_period_start` de l'abonnement + `getCycleStart()` (anniversaire mensuel, clamp fin de mois) — pas de reset au « 1er du mois »

### Demo `/api/analyze-demo`
- Table `visitor_analyses` Supabase — 1 analyse par IP (insert APRÈS le succès OpenAI : un échec ne brûle pas la démo). Le rapport est stocké avec l'IP (colonne `report`) : IP déjà marquée → RE-LIVRAISON du rapport stocké en 200 (OpenAI jamais rappelé) ; check anti-abus fail-open (échec select = console.error + continue)
...
|---|---|
| `NEXT_PUBLIC_SUPABASE_URL` | `lib/supabase.ts`, `proxy.ts`, `api/auth/callback`, `api/customer-portal`, `next.config.mjs` |
...
// Succès → upsert profil users, puis redirect /dashboard ou /pricing selon le statut
```
